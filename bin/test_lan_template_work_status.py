...
class LanTemplateWorkStatusTests(unittest.TestCase):
    ...
    def test_repair_record_skips_retired_fields_and_writes_active_suffix_fields(self):
        ...
        self.assertEqual(summary_fields["流程"], "处理中")
    ...
    def test_repair_create_request_sends_only_physical_suffix_fields(self):
        ...
        service._create_record_fields(
            ...
            fields={"维修名称": "测试维修", "所属专业": "暖通", "流程": "处理中"},
        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [
                    ...
                    for item in campus["ongoing"]
                ],
                ["c3"],
            )
            self.assertTrue(campus["ongoing"][0]["recovered_from_source"])
    ...
    def test_started_repair_source_record_is_displayed_as_update_candidate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["ongoing"][0]["source_record_id"], "r1")
            self.assertEqual(
                result["ongoing"][0]["target_record_id"],
                "target-r1",
            )
            self.assertTrue(result["ongoing"][0]["recovered_from_source"])
    ...
    def test_query_records_recovers_source_ongoing_change_without_qt_item(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = self._new_temp_service(Path(tmp))
            service._change_records = [
                _build_change_record(
            ...
            self.assertEqual(len(result["ongoing"]), 1)
            recovered = result["ongoing"][0]
            self.assertEqual(
                recovered["source_record_id"], "source-change-recovered"
            )
            self.assertEqual(recovered["work_type"], WORK_TYPE_CHANGE)
            self.assertEqual(recovered["status"], "进行中")
            self.assertTrue(recovered["active_item_id"])
            self.assertEqual(recovered.get("target_record_id", ""), "")
            self.assertEqual(recovered.get("record_id", ""), "")
    ...
    def test_query_records_recovers_source_ongoing_repair_without_qt_item(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = self._new_temp_service(Path(tmp))
            service._repair_records = [
                _build_repair_record(
            ...
            recovered = result["ongoing"][0]
            self.assertEqual(
                recovered["source_record_id"], "source-repair-recovered"
            )
            self.assertEqual(recovered["work_type"], WORK_TYPE_REPAIR)
            self.assertEqual(recovered["status"], "进行中")
            self.assertTrue(recovered["active_item_id"])
            self.assertEqual(recovered.get("target_record_id", ""), "")
...
""".strip(),
        ...
        cases = [
            ...
            (
                ...
                "设备检修",
                {
                    ...
                    "expected_time": "2026-06-25T18:00",
                    "location": "E楼",
                    "urgency": "低",
                    "repair_device": "测试设备",
                    "fault": "测试故障",
    ...
    def test_workbench_lite_blank_detail_form_defaults_to_manual(self):
        ...
        self.assertIn("绑定待发起事项", html)
        self.assertIn("不绑定", html)

    def test_workbench_source_rows_use_local_status_and_disable_non_pending_items(self):
        ...
        self.assertEqual(html.count('class="notice-row is-disabled"'), 2)
        self.assertEqual(html.count('aria-disabled="true"'), 2)
        self.assertIn("该事项已结束，只保留查看状态，不可再次发起。", html)
        self.assertIn("该事项已在“已开始未结束”中", html)
    ...
    def test_manual_source_options_only_return_pending_unlinked_items(self):
        ...
        records = [pending, local_ended, remote_ended, linked_ongoing]
        summaries = {
            "repair-local-ended": {
                "status": "已结束",
                "ended_at": "2026-07-18 12:00",
        ...
        self.assertEqual(
            [item["source_record_id"] for item in options],
            ["repair-pending"],
        )
    ...
    def test_manual_notice_command_requires_explicit_source_binding_choice(self):
        ...
        with self.assertRaisesRegex(PortalError, "必须选择绑定待发起事项或不绑定"):
    ...
    def test_repair_management_notice_prefill_maps_current_project_fields(self):
        ...
        service._ensure_repair_management_record_in_scope = (  # type: ignore[method-assign]
            lambda *_args, **_kwargs: {
                ...
                "raw_fields": {"设备检修关联": "rec_repair_source"},
            }
        )
    ...
    def test_repair_management_event_prefill_maps_event_fields_and_link(self):
        ...
        self.assertEqual(fields["故障发生现象描述"], "压缩机高压保护触发")
        self.assertEqual(fields["所属专业"], "暖通")
        self.assertEqual(fields["所属数据中心/楼栋-使用"], "南通E楼")

    def test_repair_management_event_item_prefers_fault_phenomenon_field(self):
        ...
        self.assertEqual(item["fault_phenomenon"], "压缩机高压保护触发")
    ...
    def test_repair_management_sync_adds_missing_warranty_option(self):
        ...
        meta_by_name = {warranty_meta.field_name: warranty_meta}
        captured = {}
        ensured = {}
        service._ensure_repair_management_record_in_scope = (  # type: ignore[method-assign]
        ...
        service._load_repair_management_project_records = (  # type: ignore[method-assign]
            lambda **_kwargs: ([warranty_meta], meta_by_name, [])
        )
        service._load_repair_followups_for_summary = (  # type: ignore[method-assign]
            lambda *_args, **_kwargs: ([], {}, [])
        )
        ...
        self.assertEqual(captured["fields"]["是否质保期内"], "是")
