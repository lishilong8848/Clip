...
class LanTemplateWorkStatusTests(unittest.TestCase):
    ...
    def test_repair_snapshot_does_not_mark_end_time_alone_as_completed(self):
        ...
        self.assertEqual(payload["status"], "维修中")
    ...
    def test_repair_record_skips_retired_fields_and_writes_active_suffix_fields(self):
        ...
        self.assertEqual(
            summary_fields["检修通告名称-L"],
            "EA118机房A楼UPS检修",
        )
        self.assertEqual(
            summary_fields["维修进展描述-L"],
            "检修完成，设备运行正常",
        )
    ...
    def test_repair_management_title_skips_placeholder_formula_value(self):
        record = {
            "record_id": "rec_event_repair",
            "display_fields": {
                "维修名称": "-",
                "事件描述": "E楼压缩机高压报警",
                "故障维修原因": "BMS报压缩机高压报警",
        ...
        self.assertEqual(
            MaintenancePortalService._repair_management_title(record),
            "E楼压缩机高压报警",
        )

    def test_repair_notice_title_skips_generic_placeholder_title(self):
        service = _TestMaintenancePortalService()

        self.assertEqual(
            service._repair_notice_title(
                {
    ...
    def test_repair_project_patch_updates_workbench_source_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            service._repair_records = [
                {
                    "record_id": "rec_repair_source",
                    "display_fields": {"检修通告名称": "E楼空调检修"},
                    "raw_fields": {"检修通告名称": "E楼空调检修"},
                }
            ]
            with patch.object(
                service._state_store,
                "patch_active_source_record_fields",
                return_value=1,
            ) as patch_snapshot:
                service._upsert_repair_snapshot_fields(
                    source_key=REPAIR_SNAPSHOT_SOURCE_PROJECTS,
                    record_id="rec_repair_source",
                    fields={"关联事件单": ["rec_event_linked"]},
                )

            self.assertEqual(
                service._repair_records[0]["display_fields"]["关联事件单"],
                ["rec_event_linked"],
            )
            self.assertEqual(
                service._repair_records[0]["raw_fields"]["关联事件单"],
                ["rec_event_linked"],
            )
            patch_snapshot.assert_called_once_with(
    ...
    def test_repair_management_operation_list_filters_type_and_status(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = LanPortalStateStore(Path(tmp) / "lan_portal_state.sqlite3")
            store.begin_repair_management_operation(
                "repair-sync-1",
                ...
                scope="E",
                payload_hash="sync-hash",
                summary_record_id="rec-summary",
            )
            store.update_repair_management_operation(
                "repair-sync-1",
                status="sync_pending",
                result={"available_at": 1},
            )
            store.begin_repair_management_operation(
                "repair-create-1",
                operation_type="followup_create",
                scope="E",
                payload_hash="create-hash",
            )

            operations = store.list_repair_management_operations(
                operation_types=("followup_summary_sync",),
                statuses=("sync_pending",),
            )

            self.assertEqual(len(operations), 1)
            self.assertEqual(operations[0]["operation_id"], "repair-sync-1")
            self.assertEqual(operations[0]["summary_record_id"], "rec-summary")

    def test_repair_summary_sync_task_is_persisted_and_completed(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = _TestMaintenancePortalService()
            ...
            self.assertEqual(calls[0]["summary_record_id"], "rec-summary")
            self.assertEqual(completed["status"], "completed")

    def test_repair_notice_sync_tasks_are_distinct_per_action(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = _TestMaintenancePortalService()
            service._state_store = LanPortalStateStore(  # type: ignore[assignment]
                Path(tmp) / "lan_portal_state.sqlite3"
            ...
            start_id = service._schedule_repair_sync_task(
                "notice_summary_sync",
                summary_record_id="rec-summary",
                scope="E",
                target_record_id="rec-target",
                task_payload={
                    "prepared": {"work_type": WORK_TYPE_REPAIR},
                    "action": "start",
                    "target_record_id": "rec-target",
                },
            )
            end_id = service._schedule_repair_sync_task(
                "notice_summary_sync",
                summary_record_id="rec-summary",
                scope="E",
                target_record_id="rec-target",
    ...
    def test_repair_sync_task_does_not_overwrite_newer_reschedule(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def reschedule_while_running(**_kwargs):
                time.sleep(0.002)
                service._schedule_repair_sync_task(
                    "followup_summary_sync",
                    summary_record_id="rec-summary",
                    scope="E",
                    error="新一轮同步请求",
            ...
            service._sync_repair_management_from_followup = (  # type: ignore[method-assign]
                reschedule_while_running
            )

            stats = service.process_due_repair_sync_tasks()
            latest = service._state_store.get_repair_management_operation(
                operation_id
            )

            self.assertEqual(stats["completed"], 0)
            self.assertEqual(stats["rescheduled"], 1)
            self.assertEqual(latest["status"], "sync_pending")
            self.assertEqual(latest["last_error"], "新一轮同步请求")

    def test_repair_followup_schema_read_does_not_create_missing_field(self):
        service = _TestMaintenancePortalService()
        other = FieldMeta(
            "fld_other",
            "维修名称",
        ...
        service._load_table_fields = (  # type: ignore[method-assign]
            lambda **_kwargs: ([other], {other.field_name: other})
        )
        create_calls = []
        service._request_payload = (  # type: ignore[method-assign]
            lambda *_args, **_kwargs: create_calls.append(True) or {"code": 0}
        )

        with self.assertRaisesRegex(PortalError, "启动初始化"):
            service._ensure_repair_followup_parent_id_field()
    ...
    def test_repair_records_keep_placeholder_title_when_business_fields_are_valid(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = self._new_temp_service(Path(tmp))
            service._repair_records = [
    ...
    def test_ongoing_callback_failure_is_exposed_as_runtime_warning(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                PortalRuntime.state_store.list_qt_active_items = (  # type: ignore[method-assign]
                    lambda **_kwargs: (_ for _ in ()).throw(
    ...
    def test_workbench_lite_pending_repair_can_prefill_from_incomplete_event_project(self):
        ...
        self.assertIn("if (draftComplete && !formChangedWhileSaving)", html)
        ...
        self.assertIn("function closeRepairEventCandidates(force = false)", html)
        self.assertIn("正在保存事件关联，请稍候", html)
        self.assertIn(
            "const hasOngoingBinding = Boolean(activeItemId || targetRecordId) && !targetEnded;",
            html,
        )
        self.assertIn("} else if (hasOngoingBinding) {", html)
        self.assertIn("target_ended: isEndedCandidate(candidate)", html)
        ...
        self.assertIn("function repairEventSourceRecordId(form)", html)
        self.assertNotIn(
            "panel.hidden || form.dataset.detailMode !== 'source'",
            html,
        )
        self.assertIn("active_item_id: activeItemIdAtStart", html)
        self.assertIn("target_record_id: targetRecordIdAtStart", html)
        self.assertNotIn(
            "该通告已经开始，请在发送开始前完成对应事件关联",
            html,
        )
        self.assertIn("data-source-event-id", html)
        self.assertIn("事件已关联", html)
        self.assertIn("refreshTaskInboxAfterIdentityBinding().catch", html)
        self.assertIn(
...
""".strip(),
    ...
    def test_manual_change_and_repair_do_not_require_source_records(self):
        ...
        linked_repair = service.prepare_repair_action(
            {
                "manual": True,
                "manual_id": "manual:repair:linked",
                "action": "start",
                "scope": "A",
                "source_record_id": "rec_repair_work_order",
                "repair_management_record_id": "rec_repair_work_order",
                "title": "手动绑定维修单检修",
                "building": "A楼",
                "specialty": "暖通",
                "expected_time": "2026-05-15T23:50",
        ...
        self.assertTrue(linked_repair["manual"])
        self.assertEqual(
            linked_repair["repair_management_record_id"],
            "rec_repair_work_order",
        )
        self.assertEqual(
            linked_repair["source_app_token"],
            REPAIR_SOURCE_APP_TOKEN,
        )
        self.assertEqual(
            linked_repair["source_table_id"],
            REPAIR_MANAGEMENT_TABLE_ID,
        )
    ...
    def test_target_binding_fills_fields_without_overwriting_source_record_id(self):
        ...
        self.assertNotIn("setLiteFormDirty(appliedFieldCount > 0)", handler)
    ...
    def test_manual_source_options_only_return_pending_unlinked_items(self):
        ...
        linked_pending = _build_repair_record(
            "repair-linked-pending",
            title="D楼已有目标检修记录",
            building="D楼",
            target_record_id="rec_target_existing",
        )
    ...
    def test_manual_repair_source_binding_prefills_form_from_work_order(self):
        source = (
            BIN_DIR / "lan_bitable_template_portal" / "workbench_lite.py"
        ).read_text(encoding="utf-8")
        handler = source.split(
            "async function confirmManualSourceCandidate()", 1
        )[1].split("let liteRepairEventCandidates", 1)[0]

        self.assertIn("/api/workbench/repair-event-prefill", handler)
    ...
    def test_workbench_lite_active_title_falls_back_to_notice_text(self):
        ...
        self.assertEqual(
            _record_title(
                {
                    "record_id": "rec-repair-title",
                    "work_type": "repair",
                    "title": "EA118_C01机房检修",
                    "display_fields": {
                        "检修通告名称": "EA118_C01机房E楼压缩机高压报警检修",
                        "事件描述": "E楼压缩机高压报警",
                    },
                }
            ),
            "EA118_C01机房E楼压缩机高压报警检修",
        )
    ...
    def test_manual_notice_command_requires_explicit_source_binding_choice(self):
        ...
        bound_payload["source_record_id"] = "rec_repair_pending_source"
        bound_payload["record_id"] = "rec_repair_pending_source"
        bound_payload["patch"]["source_record_id"] = "rec_repair_pending_source"
        bound_payload["patch"]["record_id"] = "rec_repair_pending_source"
        bound_payload["patch"]["manual_binding_choice"] = "bind"
        with patch.object(
            service,
            "validate_manual_source_binding",
            return_value={"source_record_id": "rec_repair_pending_source"},
        ) as validate_binding, patch.object(
            service,
            "repair_management_notice_prefill",
            return_value={
                "repair_management_record_id": "rec_repair_pending_source",
                "source_record_id": "rec_repair_pending_source",
                "target_record_id": "",
                "draft": {
                    "title": "维修单中的检修标题",
                    "location": "D-101配电室",
                    "specialty": "电气",
                    "repair_device": "D-101-UPS",
                },
            },
        ) as repair_prefill:
        ...
        repair_prefill.assert_called_once_with(
            "rec_repair_pending_source",
            scope="D",
        )
        self.assertEqual(
        ...
        self.assertEqual(expanded_bound.get("title"), "D楼纯手填检修")
        self.assertEqual(expanded_bound.get("location"), "D-101配电室")
        self.assertEqual(expanded_bound.get("repair_device"), "D-101-UPS")
        self.assertEqual(
            expanded_bound.get("source_app_token"),
            REPAIR_SOURCE_APP_TOKEN,
        )
        self.assertEqual(
            expanded_bound.get("source_table_id"),
            REPAIR_MANAGEMENT_TABLE_ID,
        )
    ...
    def test_repair_management_record_payload_requires_end_and_completed_followup(self):
        service = _TestMaintenancePortalService()

        end_only_payload = service._repair_management_record_payload(
            {
        ...
        self.assertEqual(end_only_payload["workflow"], "维修中")
        self.assertFalse(end_only_payload["is_completed"])
        self.assertFalse(end_only_payload["read_only"])
        self.assertEqual(end_only_payload["progress_percent"], 0)

        completed_payload = service._repair_management_record_payload(
            {
                "record_id": "rec_repair_completed",
                "display_fields": {
                    "维修名称": "满足完成条件的维修项目",
                    "维修结束时间（2026）": "2026-07-20 17:39",
                },
    ...
    def test_repair_management_created_date_never_forces_completion(self):
        ...
        boundary_payload = service._repair_management_record_payload(
            {
                "record_id": "rec_repair_boundary",
                "created_time": "2026-07-21 00:00",
                "display_fields": {"维修名称": "新规则维修项目"},
                "raw_fields": {"流程-L": "维修中"},
            },
            authoritative_followups=[],
        )
    ...
    def test_repair_management_records_fail_closed_when_followups_cannot_be_verified(self):
        service = _TestMaintenancePortalService()
        title_meta = FieldMeta("fld_title", "维修名称", "Text", 1, True, {}, [], False)
        service._load_table_fields = (  # type: ignore[method-assign]
            lambda **_kwargs: ([title_meta], {title_meta.field_name: title_meta})
        ...
        service._load_table_records = (  # type: ignore[method-assign]
            lambda **_kwargs: [
                {
                    "record_id": "rec_repair_unverified",
                    "display_fields": {
                        "维修名称": "跟进状态无法确认的项目",
                        "当前维修进度": "100%",
                        "维修结束时间（2026）": "2026-07-20 17:39",
        ...
        service._load_repair_followup_snapshot = (  # type: ignore[method-assign]
            lambda **_kwargs: (_ for _ in ()).throw(PortalError("跟进快照不可用"))
        )
        ...
        self.assertTrue(
            any("维修跟进状态暂未确认" in item for item in payload["schema_warnings"])
        )

    def test_repair_management_record_detail_fails_closed_when_followups_cannot_be_verified(self):
        service = _TestMaintenancePortalService()
        title_meta = FieldMeta("fld_title", "维修名称", "Text", 1, True, {}, [], False)
        project = {
            "record_id": "rec_repair_detail_unverified",
            "display_fields": {
                "维修名称": "详情跟进状态无法确认",
                "当前维修进度": "100%",
                "维修结束时间（2026）": "2026-07-20 17:39",
        ...
        payload = service.get_repair_management_record(project["record_id"])
    ...
    def test_repair_management_status_separates_active_and_completed_projects(self):
        ...
        projects = [
            ...
            },
            {
                ...
                "display_fields": {
                    ...
                    "流程": "维修完成",
                    "维修结束时间（2026）": (
                        f"{dt.datetime.now().astimezone():%Y-%m-%d} 12:00"
                    ),
                },
                "raw_fields": {},
            },
        ...
        in_progress_record = next(
            ...
            if item["record_id"] == "rec_in_progress"
        )
        self.assertEqual(in_progress_record["progress_percent"], 50)
    ...
    def test_repair_management_scope_overview_uses_project_and_followup_state(self):
        ...
        followups = [
            ...
            },
            {
                ...
                "display_fields": {
                    REPAIR_FOLLOWUP_PARENT_ID_FIELD_NAME: "rec_a_completed",
                    "维修进度": "100%",
                },
                "raw_fields": {
                    REPAIR_FOLLOWUP_PARENT_ID_FIELD_NAME: "rec_a_completed",
    ...
    def test_repair_notice_target_binding_updates_project_and_reuses_projection(self):
        service = _TestMaintenancePortalService()
        project_record = {
            "record_id": "rec_repair_summary",
            "raw_fields": {"关联事件单": ["rec_repair_event"]},
            "display_fields": {},
        }
        service._ensure_repair_management_record_in_scope = (  # type: ignore[method-assign]
        ...
        service.update_repair_management_record = (  # type: ignore[method-assign]
            lambda record_id, fields, **kwargs: (
                update_calls.append(
                    {"record_id": record_id, "fields": fields, **kwargs}
                )
                or {
                    "fields": {"设备检修关联": "rec_repair_target"},
                    "warnings": [],
                    "relation_sync": {
                        "target_results": [
                            {
                                "target_record_id": "rec_repair_target",
                                "summary_record_id": "rec_repair_summary",
                                "synced": False,
                                "already_synced": True,
                                "warning": "",
                            }
        ...
        service._sync_repair_notice_relation_projection = (  # type: ignore[method-assign]
            lambda **_kwargs: (_ for _ in ()).throw(
                AssertionError("已有关系投影时不应重复推送")
            )
        )

        payload = service.bind_repair_notice_target(
            summary_record_id="rec_repair_summary",
            target_record_id="rec_repair_target",
            active_item_id="local_repair_active",
            scope="E",
        )

        self.assertTrue(payload["saved"])
        self.assertEqual(payload["event_record_id"], "rec_repair_event")
        self.assertEqual(update_calls[0]["source_event_id"], "rec_repair_event")
        self.assertEqual(
            update_calls[0]["source_repair_ids"],
            ["rec_repair_target"],
        )
        self.assertFalse(update_calls[0]["replace_source_relations"])
        self.assertFalse(update_calls[0]["validate_required"])

    def test_repair_notice_event_binding_links_pure_manual_ongoing_to_project(self):
        service = _TestMaintenancePortalService()
        candidate_record = {
            "record_id": "rec_candidate_project",
            "display_fields": {
        ...
        service._ensure_repair_management_record_in_scope = (  # type: ignore[method-assign]
            lambda record_id, _scope, **_kwargs: candidate_record
        )
        service._event_snapshot_record_for_repair = (  # type: ignore[method-assign]
            lambda **_kwargs: {
                "source_record_id": "rec_event_bind",
        ...
        service.update_repair_management_record = (  # type: ignore[method-assign]
            lambda record_id, fields, **kwargs: (
                ...
                or {
                    "fields": {"关联事件单": "rec_event_bind"},
                    "warnings": [],
                    "relation_sync": {
                        "target_results": [
                            {
                                "target_record_id": "rec_target_repair",
                                "summary_record_id": "rec_candidate_project",
                                "synced": False,
                                "already_synced": True,
                                "warning": "",
                            }
        ...
        payload = service.bind_repair_notice_event(
            ...
            event_record_id="rec_event_bind",
            candidate_project_record_id="rec_candidate_project",
            active_item_id="local_active_repair",
            target_record_id="rec_target_repair",
            scope="E",
        )

        self.assertEqual(payload["source_record_id"], "rec_candidate_project")
        self.assertEqual(payload["target_record_id"], "rec_target_repair")
        self.assertEqual(payload["active_item_id"], "local_active_repair")
        self.assertEqual(update_calls[0]["record_id"], "rec_candidate_project")
        self.assertEqual(
            update_calls[0]["source_repair_ids"],
            ["rec_target_repair"],
        )
        self.assertEqual(
            projection_calls[0],
            {
                "summary_record_id": "rec_candidate_project",
                "event_record_id": "rec_event_bind",
                "target_record_ids": ["rec_target_repair"],
                "active_item_id": "local_active_repair",
            },
        )
    ...
    def test_repair_notice_actions_sync_project_link_title_and_action_times(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            service.sync_repair_management_notice_action(
                {
                    **prepared,
                    "progress": "设备检查中，已完成一半",
                    "response_time": "2026-07-18 10:30",
                },
                action="update",
                target_record_id="rec_repair_target",
            )
            ...
            self.assertEqual(
                set(update_fields),
                {"设备检修关联", "检修通告名称", "维修进展描述"},
            )
            self.assertEqual(
                update_fields["维修进展描述"],
                "设备检查中，已完成一半",
            )
            ...
            service.sync_repair_management_notice_action(
                {
                    **prepared,
                    "progress": "检修完成，设备运行正常",
                    "response_time": "2026-07-18 11:45",
                },
                action="end",
                target_record_id="rec_repair_target",
            )
    ...
    def test_repair_target_summary_id_sync_writes_missing_and_preserves_conflict(self):
        ...
        patches: list[dict] = []
        snapshots: list[dict] = []
        service._repair_management_repair_source_config = (  # type: ignore[method-assign]
            lambda: ("app_target", REPAIR_MANAGEMENT_REPAIR_TABLE_ID)
        )
        service._load_table_fields = (  # type: ignore[method-assign]
            lambda **_kwargs: ([summary_meta], {summary_meta.field_name: summary_meta})
        )
            ...
            lambda **_kwargs: [
                {
                    "record_id": "rec_repair_target",
                    "raw_fields": {
                        REPAIR_TARGET_SUMMARY_ID_FIELD_NAME: current_value["value"]
                    },
                    "display_fields": {},
                }
            ]
        ...
        written = service._sync_repair_target_summary_id(
            target_record_id="rec_repair_target",
            summary_record_id="rec_repair_summary",
        )

        self.assertTrue(written["synced"])
        self.assertEqual(
            patches[-1]["fields"],
            {REPAIR_TARGET_SUMMARY_ID_FIELD_NAME: "rec_repair_summary"},
        )
        self.assertEqual(
            snapshots[-1]["fields"],
            {REPAIR_TARGET_SUMMARY_ID_FIELD_NAME: "rec_repair_summary"},
        ...
        current_value["value"] = "rec_other_summary"
        conflicted = service._sync_repair_target_summary_id(
            target_record_id="rec_repair_target",
            summary_record_id="rec_repair_summary",
        )
    ...
    def test_repair_target_summary_id_clear_only_removes_matching_summary(self):
        service = _TestMaintenancePortalService()
        summary_meta = FieldMeta(
            "fld_summary_id",
            REPAIR_TARGET_SUMMARY_ID_FIELD_NAME,
        ...
        current_value = {"value": "rec_repair_summary"}
        patches: list[dict] = []
        service._repair_management_repair_source_config = (  # type: ignore[method-assign]
            lambda: ("app_target", REPAIR_MANAGEMENT_REPAIR_TABLE_ID)
        )
        ...
        def patch_target(**kwargs):
            patches.append(kwargs)
            current_value["value"] = kwargs["fields"][
                REPAIR_TARGET_SUMMARY_ID_FIELD_NAME
            ]
            return {"code": 0}

        service._patch_record_fields_exact = patch_target  # type: ignore[method-assign]
        service._upsert_repair_snapshot_fields = (  # type: ignore[method-assign]
            lambda **_kwargs: None
        )

        cleared = service._clear_repair_target_summary_id(
            target_record_id="rec_repair_target",
            summary_record_id="rec_repair_summary",
        )
        ...
        current_value["value"] = "rec_other_summary"
        preserved = service._clear_repair_target_summary_id(
            target_record_id="rec_repair_target",
            summary_record_id="rec_repair_summary",
        )

        self.assertFalse(preserved["cleared"])
        self.assertTrue(preserved["skipped"])
        self.assertIn("已关联其他维修单", preserved["warning"])
        self.assertEqual(len(patches), 1)

    def test_repair_project_relation_rebind_clears_only_previous_target(self):
        service = _TestMaintenancePortalService()
        ...
        service._sync_repair_target_summary_id = (  # type: ignore[method-assign]
            lambda **kwargs: (
                ...
                or {
                    "target_record_id": kwargs["target_record_id"],
        ...
        result = service._sync_repair_project_relations(
            summary_record_id="rec_repair_summary",
            target_record_ids=["rec_new_target"],
        ...
        self.assertEqual(
            ...
            [
                {
                    "target_record_id": "rec_old_target",
                    "summary_record_id": "rec_repair_summary",
                }
            ],
        )
        self.assertTrue(result["cleared_target_results"][0]["cleared"])

    def test_repair_project_relation_rejects_multiple_targets(self):
        service = _TestMaintenancePortalService()

        with self.assertRaisesRegex(PortalError, "一次只能选择一条"):
            service._sync_repair_project_relations(
                summary_record_id="rec_repair_summary",
                target_record_ids=["rec_target_one", "rec_target_two"],
            )

    def test_repair_project_update_preflights_target_before_project_write(self):
        service = _TestMaintenancePortalService()
        ...
        service._ensure_repair_management_record_in_scope = (  # type: ignore[method-assign]
            lambda *_args, **_kwargs: {
                "record_id": "rec_repair_summary",
                "source_table_id": REPAIR_MANAGEMENT_TABLE_ID,
                "raw_fields": {"设备检修关联": "rec_old_target"},
                "display_fields": {},
            }
        )
        service._sync_repair_target_summary_id = (  # type: ignore[method-assign]
        ...
        with self.assertRaisesRegex(PortalError, "已关联其他维修单"):
            service.update_repair_management_record(
                "rec_repair_summary",
                {},
                source_repair_ids=["rec_new_target"],
    ...
    def test_repair_relation_projection_updates_ongoing_and_identity(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = self._new_temp_service(Path(tmp))
            service._state_store.upsert_qt_active_item(
                {
            ...
            service._state_store.upsert_qt_active_item(
                {
                    "active_item_id": "other_repair_active",
                    "record_id": "rec_other_repair_target",
                    "target_record_id": "rec_other_repair_target",
                    "source_record_id": "rec_repair_summary",
                    "work_type": WORK_TYPE_REPAIR,
                    "notice_type": "设备检修",
                    "title": "E楼另一条检修",
            ...
            result = service._sync_repair_notice_relation_projection(
                summary_record_id="rec_repair_summary",
                event_record_id="rec_repair_event",
                target_record_ids=["rec_repair_target"],
                active_item_id="local_repair_active",
            )

            self.assertEqual(result["active_updated"], 1)
            ...
            active_payload = next(
                ...
                if row["active_item_id"] == "local_repair_active"
            )
            self.assertEqual(
            ...
            self.assertEqual(
                active_payload["repair_management_record_id"],
                "rec_repair_summary",
            )
            self.assertEqual(
                active_payload["source_event_id"],
                "rec_repair_event",
            )
            unrelated_payload = next(
                row["payload"]
                for row in active_rows
                if row["active_item_id"] == "other_repair_active"
            )
            self.assertNotIn("source_event_id", unrelated_payload)
    ...
    def test_repair_relation_projection_replaces_stale_target_for_explicit_active(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = self._new_temp_service(Path(tmp))
            service._state_store.upsert_qt_active_item(
                {
                    "active_item_id": "local_repair_active",
                    "record_id": "rec_stale_target",
                    "target_record_id": "rec_stale_target",
                    "work_type": WORK_TYPE_REPAIR,
                    "notice_type": "设备检修",
                    "title": "E楼精密空调检修",
                    "scope": "E",
            ...
            result = service._sync_repair_notice_relation_projection(
                summary_record_id="rec_repair_summary",
                event_record_id="rec_repair_event",
                target_record_ids=["rec_selected_target"],
                active_item_id="local_repair_active",
            )

            self.assertEqual(result["active_updated"], 1)
            active_row = service._state_store.list_qt_active_items(
                include_deleted=False
            )[0]
            self.assertEqual(
                active_row["payload"]["target_record_id"],
                "rec_selected_target",
            )
            self.assertEqual(
                active_row["payload"]["record_id"],
                "rec_selected_target",
            )

    def test_repair_target_relation_confirmation_is_scoped_to_requested_target(self):
        service = _TestMaintenancePortalService()
        sync_calls: list[dict] = []
        service._sync_repair_target_summary_id = (  # type: ignore[method-assign]
            lambda **kwargs: (
        ...
        result = service._ensure_repair_target_relation_synced(
            summary_record_id="rec_repair_summary",
            target_record_id="rec_requested_target",
            relation_sync={
                "target_results": [
                    {
                        "target_record_id": "rec_other_target",
                        "summary_record_id": "rec_repair_summary",
                        "synced": True,
                        "already_synced": False,
                        "warning": "",
        ...
        self.assertEqual(
            result["target_results"][-1]["target_record_id"],
            "rec_requested_target",
        )
    ...
    def test_repair_management_end_without_followup_stays_in_progress(self):
        service = _TestMaintenancePortalService()
        reason = FieldMeta(
            "fld_reason",
            "故障维修原因",
        ...
        self.assertEqual(patch_calls, [{"故障维修原因": "已处理完成"}])
        self.assertEqual(result["workflow"], "维修中")
        self.assertTrue(result["workflow_synced"])

    def test_repair_management_unlinked_update_syncs_existing_followups(self):
        service = _TestMaintenancePortalService()
        start = FieldMeta(
            "fld_start",
            "维修开始时间",
        ...
        linked_followup = {
            ...
            "display_fields": {},
            "raw_fields": {
                REPAIR_FOLLOWUP_PARENT_ID_FIELD_NAME: "rec-unlinked"
            },
        }
        sync_calls = []
        service._load_repair_management_project_records = (  # type: ignore[method-assign]
        ...
        result = service.update_repair_management_record(
            "rec-unlinked",
            {"维修开始时间": "2026-07-23 09:00"},
            validate_required=False,
        )

        self.assertEqual(len(sync_calls), 1)
        self.assertEqual(sync_calls[0]["summary_record_id"], "rec-unlinked")
        self.assertEqual(
            sync_calls[0]["linked_followups"][0]["record_id"],
            "rec-followup",
        )
    ...
    def test_repair_management_repair_candidates_sort_by_created_time_desc(self):
        service = _TestMaintenancePortalService()
        service._event_snapshot_record_for_repair = (  # type: ignore[method-assign]
            lambda **_kwargs: {
                "source_record_id": "rec_event_e",
        ...
        self.assertEqual(payload["records"][0]["created_time"], "2026-08-01 09:00")
        self.assertNotIn("_created_time_ms", payload["records"][0])
    ...
    def test_repair_notice_syncs_followup_times_without_forcing_progress(self):
        service = _TestMaintenancePortalService()
        metas = [
            FieldMeta(
                "fld_parent",
        ...
        for update in patched:
            ...
            self.assertNotIn("维修进度", fields)
            self.assertNotIn("设备名称", fields)
            self.assertNotIn("维修进展描述", fields)
        self.assertEqual(len(snapshots), 2)

    def test_repair_project_save_does_not_complete_partial_followup(self):
        service = _TestMaintenancePortalService()
        metas = [
            FieldMeta("fld_title", "维修名称", "Text", 1, True, {}, [], False),
            FieldMeta("fld_link", "设备检修关联", "Text", 1, False, {}, [], False),
        ...
        self.assertNotIn("completed", sync_calls[-1])
        self.assertEqual(
    ...
    def test_repair_management_sync_uses_latest_followup_for_current_fields(self):
        ...
        self.assertEqual(fields["设备名称"], "A-UPS-02、A-UPS-01")
        self.assertEqual(fields["设备编号"], "A-UPS-02、A-UPS-01")
        self.assertEqual(fields["设备品牌"], "南都")
        self.assertEqual(fields["设备型号"], "6-GFM-150HR、6-GFM-140HR")
    ...
    def test_repair_followup_bind_candidates_only_include_unbound_scope_records(self):
        ...
        service._load_repair_followup_snapshot = (  # type: ignore[method-assign]
            lambda **_kwargs: (
                ...
                [
                    ...
                    {
                        "record_id": "rec_bound_e",
                        "raw_fields": {
                            REPAIR_FOLLOWUP_PARENT_ID_FIELD_NAME: "rec_other"
                        },
                        "display_fields": {
                            "维修进展描述": "已绑定记录",
                            "楼栋": "E楼",
        ...
        payload = service.list_repair_followup_bind_candidates(
            summary_record_id="rec_summary",
            scope="E",
            query="UPS",
        )
        ...
        self.assertEqual(payload["records"][0]["progress"], "50%")
        self.assertEqual(
            payload["records"][0]["repair_summary"],
            "E楼UPS月度维修跟进",
        )
        self.assertEqual(payload["records"][0]["repair_name"], "E楼UPS维修")
        self.assertEqual(payload["records"][0]["repair_order_no"], "WX-E-001")
        self.assertIn("BMS系统", payload["records"][0]["repair_source"])

    def test_repair_followup_manual_bind_writes_parent_and_syncs_summary(self):
        service = _TestMaintenancePortalService()
        parent_meta = FieldMeta(
            "fld_parent",
            REPAIR_FOLLOWUP_PARENT_ID_FIELD_NAME,
        ...
        service._load_table_records_by_ids = (  # type: ignore[method-assign]
            lambda **_kwargs: [
                {
                    "record_id": "rec_unbound",
                    "raw_fields": {},
                    "display_fields": {"楼栋": "E楼"},
                },
                {
        ...
        service._sync_repair_management_from_followup = (  # type: ignore[method-assign]
            lambda **kwargs: sync_calls.append(
                (
                    str(kwargs.get("summary_record_id") or ""),
        ...
        result = service.bind_repair_followup_records(
            summary_record_id="rec_summary",
            followup_record_ids=["rec_unbound", "rec_current", "rec_unbound"],
            scope="E",
        )

        self.assertEqual(result["bound_record_ids"], ["rec_unbound"])
        self.assertEqual(result["already_bound_record_ids"], ["rec_current"])
        self.assertEqual(len(patches), 1)
        self.assertEqual(
            patches[0]["fields"],
            {REPAIR_FOLLOWUP_PARENT_ID_FIELD_NAME: "rec_summary"},
        ...
        self.assertEqual(sync_calls, [("rec_summary", "E")])

    def test_repair_followup_manual_bind_rejects_record_owned_by_other_summary(self):
        service = _TestMaintenancePortalService()
        parent_meta = FieldMeta(
            "fld_parent",
            REPAIR_FOLLOWUP_PARENT_ID_FIELD_NAME,
        ...
        with self.assertRaisesRegex(PortalError, "已绑定其他维修单"):
            service.bind_repair_followup_records(
                summary_record_id="rec_summary",
                followup_record_ids=["rec_owned"],
                scope="E",
            )
    ...
    def test_repair_management_sync_adds_missing_warranty_option(self):
        ...
        followup_link_meta = FieldMeta(
            "fld_followup_links_l",
            "维修跟进记录",
            "Text",
            1,
            False,
        ...
        meta_by_name = {
            ...
            followup_link_meta.field_name: followup_link_meta,
        }
        captured: list[dict] = []
        ensured = {}
        service._ensure_repair_management_record_in_scope = (  # type: ignore[method-assign]
        ...
        service._load_repair_management_project_records = (  # type: ignore[method-assign]
            lambda **_kwargs: (
                [warranty_meta, workflow_meta, cmdb_meta, followup_link_meta],
                meta_by_name,
                [],
            )
        )
    ...
    def test_repair_followup_backfill_links_unique_summary_and_writes_backlink(self):
        ...
        summary_patch = next(
            item
            for item in patches
            if item["table_id"] == REPAIR_MANAGEMENT_TABLE_ID
            and item["record_id"] == "rec_summary"
        )
        stale_summary_patch = next(
            item
            for item in patches
            if item["table_id"] == REPAIR_MANAGEMENT_TABLE_ID
            and item["record_id"] == "rec_summary_stale"
        )
        self.assertEqual(
        ...
        self.assertIsNone(stale_summary_patch["fields"]["维修跟进记录"])
    ...
    def test_repair_integrity_marks_blank_orphan_as_manual_review_only(self):
        ...
        service._state_store.get_repair_snapshot = (  # type: ignore[method-assign]
            lambda source_key: snapshots[source_key]
        )
        ...
        start_result = service.start_repair_management_integrity_repair(
            scope="ALL"
        )

        self.assertEqual(integrity["issue_count"], 1)
        self.assertEqual(integrity["repairable_issue_count"], 0)
        self.assertEqual(integrity["manual_review_count"], 1)
        self.assertEqual(integrity["unidentified_followup_count"], 1)
        self.assertEqual(start_result["status"], "not_needed")
        self.assertFalse(start_result["started"])

    def test_repair_integrity_only_patches_unique_relation_ids(self):
        service = _TestMaintenancePortalService()
        snapshots = {
            REPAIR_SNAPSHOT_SOURCE_PROJECTS: {
                "exists": True,
        ...
        def upsert_snapshot_fields(*, source_key, record_id, fields):
            for record in snapshots[source_key]["records"]:
                if record["record_id"] != record_id:
                    continue
                record.setdefault("raw_fields", {}).update(fields)
                record.setdefault("display_fields", {}).update(fields)
                return

        service._upsert_repair_snapshot_fields = (  # type: ignore[method-assign]
            upsert_snapshot_fields
        )
        service._rebuild_repair_project_status_index = (  # type: ignore[method-assign]
            lambda: {"status": "ready"}
        )

        result = service.repair_repair_management_integrity(scope="ALL")

        self.assertEqual(result["status"], "complete")
        self.assertEqual(result["before_repairable_issue_count"], 1)
        self.assertEqual(result["after_repairable_issue_count"], 0)
