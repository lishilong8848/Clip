...
def _attachment_items(record: dict[str, Any], *keys: str) -> list[dict[str, Any]]:
    ...
    for key in keys:
        ...
        for raw in items:
            ...
            normalized = {
                "local_image_id": str(item.get("local_image_id") or "").strip(),
                ...
                "target_written": bool(item.get("target_written")),
            }
            ...
            identity = normalized["local_image_id"] or normalized["file_token"] or normalized["upload_id"] or normalized["preview_url"] or normalized["file_name"]
...
def _polling_work_order_selector(work_type: str) -> str:
    ...
    return f"""
      <section class="form-section polling-work-order-select" data-polling-work-order-select>
        <header><strong>{_e(title)}</strong><span id="lite-polling-selection-summary">未选择 SOP</span></header>
        <div class="polling-work-order-actions">
          <button class="btn ghost" id="lite-polling-select-open" type="button">选择操作步骤</button>
          <label class="polling-work-order-exempt"><input id="lite-polling-work-order-exempt" name="polling_work_order_exempt" type="checkbox" value="1"><span>{_e(exempt)}</span></label>
        </div>
      </section>
    """
...
def _polling_work_order_status(source: dict[str, Any], work_type: str) -> str:
    ...
    return f"""
      <section class="form-section polling-work-order-status" data-polling-group-id="{_e(group_id)}">
...
def _record_rows(
    ...
    *,
    ...
    for record in sorted(records, key=_record_sort_key):
        ...
        site_photo_images = _attachment_items(row_source, "local_site_images")
        ali_confirmation_images = _attachment_items(row_source, "local_ali_confirmation_images")
...
def _ongoing_rows(
    ...
    *,
    ...
    for item in filtered:
        ...
        site_photo_images = _attachment_items(item, "local_site_images")
        ali_confirmation_images = _attachment_items(item, "local_ali_confirmation_images")
        ali_confirmation_count = max(
...
def _detail_form(
    *,
    ...
    site_photo_images = _attachment_items(source, "local_site_images")
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; }}
    ...
    .site-photo-preview {{ flex:0 0 auto; border:0; border-radius:9px; padding:0; background:transparent; cursor:zoom-in; }}
    .site-photo-preview:focus-visible {{ outline:2px solid #1f63ff; outline-offset:2px; }}
    ...
    .end-check-actions .job-status {{ align-self:center; margin-right:auto; }}
    ...
    .change-confirmation-upload-zone {{ min-width:190px; display:grid; grid-template-columns:48px minmax(0,1fr); gap:8px; align-items:center; border:1px dashed #93c5fd; border-radius:12px; padding:6px; color:#53677f; background:#f8fbff; cursor:pointer; }}
    .change-confirmation-upload-zone.active,.change-confirmation-upload-zone:focus-within {{ border-color:#1f63ff; background:#eef6ff; box-shadow:0 0 0 2px rgba(31,99,255,.1); outline:0; }}
    .change-confirmation-upload-zone input {{ display:none; }}
    .change-confirmation-upload-zone .site-photo-add-icon {{ display:grid; place-items:center; width:44px; height:44px; border-radius:9px; background:#fff; }}
    .change-confirmation-upload-copy {{ min-width:0; font-size:11px; font-weight:850; line-height:1.35; }}
    .change-confirmation-upload-copy b,.change-confirmation-upload-copy span {{ display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }}
    .change-confirmation-upload-copy b {{ color:#0a57d8; }}
    .change-confirmation-upload-copy span {{ color:#64748b; }}
    .change-confirmation-local-missing {{ margin-top:8px; color:#8a4b00; font-size:11px; font-weight:850; }}
    ...
    .polling-sop-row:disabled {{ cursor:not-allowed; opacity:.58; }}
    ...
    .polling-step-edit {{ min-width:0; border:1px solid #d8e5f7; border-radius:8px; padding:12px; background:#fff; }}
    .polling-step-edit header {{ display:flex; align-items:center; flex-wrap:wrap; gap:8px; margin-bottom:9px; }}
    .polling-step-edit header > strong {{ color:#0c244d; font-size:14px; }}
    .polling-step-edit textarea {{ min-height:76px; resize:vertical; }}
    ...
    .polling-step-time input {{ width:88px; min-height:32px; padding:4px 7px; }}
    .polling-step-remove {{ min-height:32px; padding:5px 9px; font-size:11px; }}
    .polling-step-settings {{ display:grid; gap:0; margin-top:10px; border-top:1px solid #e2eaf6; }}
    .polling-delay-control {{ min-height:52px; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px; padding:9px 2px; }}
    .polling-delay-control label {{ margin:0; }}
    .polling-delay-toggle {{ display:inline-flex; align-items:center; gap:8px; color:#334c69; font-size:12px; font-weight:850; cursor:pointer; }}
    .polling-delay-toggle input {{ width:17px; min-height:17px; margin:0; accent-color:#b7791f; }}
    .polling-delay-control.active .polling-delay-toggle {{ color:#935314; }}
    .polling-delay-minutes {{ display:inline-flex; align-items:center; gap:6px; color:#64748b; font-size:11px; font-weight:800; }}
    .polling-delay-minutes input {{ width:72px; min-height:32px; padding:4px 7px; color:#8a4b00; border-color:#edcf9a; background:#fffbf1; }}
    .polling-delay-minutes[hidden] {{ display:none; }}
    .polling-loop-control {{ display:grid; gap:7px; border-top:1px solid #e2eaf6; padding-top:10px; }}
    .polling-loop-head {{ display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px; min-height:32px; }}
    .polling-loop-head strong {{ color:#334c69; font-size:12px; }}
    .polling-loop-head small {{ margin-left:5px; color:#64748b; font-size:11px; font-weight:750; }}
    .polling-loop-add {{ min-height:32px; border:1px solid #c8ddf8; border-radius:7px; padding:5px 10px; color:#0a57d8; background:#f6faff; font-size:11px; font-weight:850; cursor:pointer; }}
    .polling-loop-add:hover,.polling-loop-add:focus-visible {{ border-color:#7aaff7; background:#eaf3ff; outline:0; }}
    .polling-repeat-rules {{ display:grid; gap:0; }}
    .polling-repeat-rule {{ display:flex; align-items:end; flex-wrap:wrap; gap:8px; border-top:1px solid #edf2f8; padding:9px 0; }}
    .polling-repeat-index {{ align-self:center; min-width:44px; color:#416184; font-size:11px; font-weight:850; }}
    .polling-repeat-field {{ min-width:110px; flex:1 1 130px; display:grid; gap:4px; margin:0; }}
    .polling-repeat-field span {{ margin:0; color:#64748b; font-size:11px; font-weight:800; }}
    .polling-repeat-field select {{ min-height:34px; }}
    .polling-repeat-count {{ min-width:76px; flex:0 0 88px; }}
    .polling-repeat-count input {{ min-height:34px; }}
    .polling-repeat-remove {{ flex:0 0 34px; width:34px; min-height:34px; border:1px solid #f3c5c5; border-radius:7px; padding:0; color:#b42318; background:#fff7f7; font-size:18px; line-height:1; cursor:pointer; }}
    .polling-repeat-remove:hover,.polling-repeat-remove:focus-visible {{ border-color:#e99292; background:#ffeded; outline:0; }}
    .polling-step-preview {{ display:grid; gap:7px; overflow-wrap:anywhere; }}
    .polling-step-preview-main {{ color:#0c244d; font-size:12px; line-height:1.5; }}
    .polling-step-preview-meta {{ display:flex; flex-wrap:wrap; gap:6px 12px; color:#53677f; font-size:11px; font-weight:800; }}
    .polling-step-preview-meta .delay {{ color:#935314; }}
    .polling-step-preview-meta .loop {{ color:#0a57d8; }}
    .polling-sop-sync-note {{ border-left:3px solid #e6ac51; padding:7px 10px; color:#875018; background:#fff9ed; font-size:12px; line-height:1.45; }}
    .polling-sop-sync-note.conflict {{ border-left-color:#e58a86; color:#a12c25; background:#fff5f4; }}
    ...
    .polling-cooling-mode[hidden] {{ display:none; }}
    ...
    .polling-sop-drop {{ display:grid; place-items:center; min-height:76px; border:1px dashed #8db5ea; border-radius:14px; background:#f5f9ff; color:#235a9f; font-weight:850; cursor:pointer; }}
    .polling-sop-drop.dragover {{ border-color:#1678ff; background:#eaf3ff; box-shadow:0 0 0 3px #1678ff20; }}
    .polling-run-row {{ display:grid; grid-template-columns:48px 1fr auto 1fr; gap:7px; align-items:center; }}
    .polling-people-grid {{ display:grid; grid-template-columns:1fr 1fr; gap:8px; }}
    .polling-person-picker {{ display:grid; align-content:start; gap:5px; min-width:0; }}
    .polling-person-results {{ display:grid; gap:4px; max-height:220px; overflow:auto; border:1px solid #cdddf2; border-radius:12px; padding:5px; background:#fff; box-shadow:0 10px 24px #1c4f8b18; }}
    .polling-person-results[hidden] {{ display:none; }}
    .polling-person-result {{ display:grid; gap:2px; width:100%; border:0; border-radius:9px; padding:7px 8px; background:#f7faff; color:#17365d; text-align:left; cursor:pointer; }}
    .polling-person-result:hover,.polling-person-result:focus {{ background:#e9f3ff; outline:2px solid #8dbdff; }}
    .polling-person-result small {{ color:#64748b; }}
    .polling-work-order-actions {{ display:flex; flex-wrap:wrap; align-items:center; gap:8px; }}
    .polling-work-order-exempt {{ min-height:38px; display:inline-flex; align-items:center; gap:7px; border:1px solid #cfe0ff; border-radius:12px; padding:7px 11px; color:#0a4fc4; background:#f4f8ff; font-size:12px; font-weight:900; cursor:pointer; }}
    .polling-work-order-exempt input {{ width:17px; min-height:17px; margin:0; accent-color:#1f63ff; }}
    .polling-work-order-exempt span {{ display:inline; margin:0; color:inherit; font-size:inherit; }}
    .polling-work-order-exempt.active {{ border-color:#1f63ff; background:#eaf3ff; box-shadow:0 0 0 2px rgba(31,99,255,.1); }}
...
<body>
  ...
  <script>
    ...
    function pollingSopWorkType() {{const editing=String(litePollingEditingSop?.work_type||'');return litePollingSopMode==='manage'&&['maintenance','adjust','polling'].includes(editing)?editing:pollingCurrentSopWorkType();}}
    ...
    async function loadPollingSops(force=false) {{
      ...
      if(!force&&cached?.items&&Date.now()-cached.loadedAt<60000&&!cached.items.some(item=>item.cloud_sync_required&&item.cloud_sync_status==='pending')){{litePollingSops=cached.items;return litePollingSops}}
    ...
    function pollingShiftFromHour(hour) {{
      return Number(hour)>=9&&Number(hour)<18?'白':'夜';
    }}
    function pollingCurrentShift() {{
    ...
    function pollingDraftStepId() {{return 'step_'+Date.now().toString(36)+Math.random().toString(36).slice(2)}}
    function pollingNewDraft(workType=pollingSopWorkType()) {{ return {{sop_id:'',work_type:workType,scope:pollingSopScope(),name:'',version:0,steps:[{{step_id:pollingDraftStepId(),content:'',operator_required:true,reviewer_required:true,photo_required:true,time_limit_seconds:0,delay_reminder_minutes:0,repeat_rules:[]}}],attachments:[]}}; }}
    ...
    function pollingStepEditor(step,index) {{
      ...
      const restoreStep=(focusSelector='')=>{{const editor=document.getElementById('lite-polling-sop-editor'),scroll=editor?.scrollTop||0;renderPollingSopEditor();if(editor)editor.scrollTop=scroll;if(focusSelector)editor?.querySelectorAll('.polling-step-edit')[index]?.querySelector(focusSelector)?.focus()}};
      const head=document.createElement('header'),title=document.createElement('strong'),limitLabel=document.createElement('label'),limitText=document.createElement('span'),limit=document.createElement('input'),remove=document.createElement('button');title.textContent=`步骤 ${{index+1}}`;limitLabel.className='polling-step-time';limitText.textContent='时间限制（秒）';limit.type='number';limit.min='0';limit.max='86400';limit.step='1';limit.value=String(Math.max(0,Number(step.time_limit_seconds||0)));limit.oninput=()=>step.time_limit_seconds=Math.max(0,Math.trunc(Number(limit.value||0)));limitLabel.append(limitText,limit);remove.type='button';remove.className='btn danger-ghost polling-step-remove';remove.textContent='删除步骤';remove.onclick=()=>{{const removedId=String(step.step_id||'');litePollingEditingSop.steps.splice(index,1);for(const item of litePollingEditingSop.steps)item.repeat_rules=(item.repeat_rules||[]).filter(rule=>rule.from_step_id!==removedId&&rule.to_step_id!==removedId);renderPollingSopEditor()}};head.append(title,limitLabel,remove);
      ...
      const settings=document.createElement('div');settings.className='polling-step-settings';
      const reminderRow=document.createElement('div'),reminderLabel=document.createElement('label'),reminderCheck=document.createElement('input'),minutesLabel=document.createElement('label'),minutes=document.createElement('input');reminderRow.className='polling-delay-control';reminderLabel.className='polling-delay-toggle';reminderCheck.type='checkbox';reminderCheck.checked=Number(step.delay_reminder_minutes||0)>0;reminderLabel.append(reminderCheck,document.createTextNode('完成后延时提醒'));minutesLabel.className='polling-delay-minutes';minutes.type='number';minutes.min='1';minutes.max='1440';minutes.step='1';minutes.value=String(Number(step.delay_reminder_minutes||0)||5);minutes.setAttribute('aria-label','延时提醒分钟数');minutesLabel.append(document.createTextNode('延时'),minutes,document.createTextNode('分钟'));const syncReminder=()=>{{minutesLabel.hidden=!reminderCheck.checked;minutes.disabled=!reminderCheck.checked;minutes.required=reminderCheck.checked;reminderRow.classList.toggle('active',reminderCheck.checked);step.delay_reminder_minutes=reminderCheck.checked?Number(minutes.value||0):0}};reminderCheck.onchange=syncReminder;minutes.oninput=()=>step.delay_reminder_minutes=reminderCheck.checked?Number(minutes.value||0):0;syncReminder();reminderRow.append(reminderLabel,minutesLabel);
    ...
    function pollingAttachmentRows(sop) {{
      ...
      for(const item of sop.attachments||[]){{const row=document.createElement('div');row.className='polling-sop-attachment';const link=document.createElement('a');link.href=item.download_url||'#';link.textContent=`${{item.name}} · ${{Math.ceil(Number(item.size||0)/1024)}}KB`;const remove=document.createElement('button');remove.type='button';remove.className='btn danger-ghost';remove.textContent='删除';remove.onclick=async()=>{{try{{const draft=litePollingEditingSop,remote=await pollingApi(`/api/polling-sops/${{encodeURIComponent(sop.sop_id)}}/attachments/${{encodeURIComponent(item.attachment_id)}}?expected_version=${{sop.version}}`,{{method:'DELETE'}});litePollingEditingSop=pollingMergeSopAttachmentResult(draft,remote);await loadPollingSops();renderPollingSopList();renderPollingSopEditor()}}catch(error){{showLiteError(error.message)}}}};row.append(link,remove);box.append(row)}}return box;
    }}
    function pollingPendingAttachmentRows() {{const box=document.createElement('div');box.className='polling-sop-attachments';litePollingPendingSopFiles.forEach((file,index)=>{{const row=document.createElement('div');row.className='polling-sop-attachment';const name=document.createElement('span');name.textContent=`待保存：${{file.name}} · ${{Math.ceil(file.size/1024)}}KB`;const remove=document.createElement('button');remove.type='button';remove.className='btn danger-ghost';remove.textContent='移除';remove.onclick=()=>{{litePollingPendingSopFiles.splice(index,1);renderPollingSopEditor()}};row.append(name,remove);box.append(row)}});return box}}
    ...
    function renderPollingSopEditor() {{
      const editor=document.getElementById('lite-polling-sop-editor');if(!editor)return;const sop=litePollingEditingSop||pollingNewDraft();litePollingEditingSop=sop;for(const step of sop.steps||[])if(!step.step_id)step.step_id=pollingDraftStepId();editor.replaceChildren();
      ...
      editor.append(workTypeField);if(sop.cloud_sync_required&&sop.cloud_sync_status!=='synced'){{const syncNote=document.createElement('div');syncNote.className='polling-sop-sync-note'+(sop.cloud_sync_status==='conflict'?' conflict':'');syncNote.textContent=sop.cloud_sync_error||'此 SOP 仍保存在本机，尚未同步到多维。';editor.append(syncNote)}}editor.append(label);if(typeLabel)editor.append(typeLabel,typeHint);editor.append(steps,actions);
    ...
    function pollingUnitNumbersFromContent(content) {{
      return Array.from(String(content||'').match(/[1-6]/g)||[],value=>`${{value}}#`).slice(0,12);
    }}
    function pollingRunsFromContent(content) {{
      const values=pollingUnitNumbersFromContent(content),runs=[],seen=new Set(),used=new Set(),add=(fromUnit,toUnit)=>{{const key=`${{fromUnit}}>${{toUnit}}`;if(fromUnit!==toUnit&&pollingUnitGroup(fromUnit).includes(toUnit)&&!seen.has(key)&&!used.has(fromUnit)&&!used.has(toUnit)){{seen.add(key);used.add(fromUnit);used.add(toUnit);runs.push({{from_unit:fromUnit,to_unit:toUnit}})}}}};if(values.length<2)return runs;if(pollingUnitGroup(values[0]).includes(values[1])){{for(let index=0;index+1<values.length&&runs.length<2;index+=2)add(values[index],values[index+1]);return runs}}const starts=values.slice(0,2),ends=[null,null];for(const value of values.slice(2)){{for(let index=0;index<starts.length;index++){{if(!ends[index]&&value!==starts[index]&&pollingUnitGroup(starts[index]).includes(value)){{ends[index]=value;break}}}}}}for(let index=0;index<starts.length;index++)if(ends[index])add(starts[index],ends[index]);return runs;
    }}
    function pollingRunRows(container,count,runs) {{
      const nodes=[],used=new Set();for(let index=0;index<count;index++){{
        const row=document.createElement('div');row.className='polling-run-row';const label=document.createElement('b');label.textContent=`第${{index+1}}次`;const from=document.createElement('select'),arrow=document.createElement('span'),to=document.createElement('select');from.setAttribute('aria-label',`第${{index+1}}次起点`);to.setAttribute('aria-label',`第${{index+1}}次终点`);arrow.textContent='→';
        const availableStarts=litePollingUnits.filter(unit=>!used.has(unit)&&pollingUnitGroup(unit).filter(candidate=>!used.has(candidate)).length>=2);for(const unit of availableStarts){{const option=document.createElement('option');option.value=unit;option.textContent=unit;from.append(option)}}
        const value=runs[index]||{{from_unit:'',to_unit:''}};runs[index]=value;from.value=availableStarts.includes(value.from_unit)?value.from_unit:(availableStarts[0]||'');const availableEnds=pollingUnitGroup(from.value).filter(unit=>unit!==from.value&&!used.has(unit));to.replaceChildren(...availableEnds.map(unit=>{{const option=document.createElement('option');option.value=unit;option.textContent=unit;return option}}));to.value=availableEnds.includes(value.to_unit)?value.to_unit:(availableEnds[0]||'');value.from_unit=from.value;value.to_unit=to.value;from.onchange=()=>{{value.from_unit=from.value;value.to_unit='';pollingRunRows(container,count,runs)}};to.onchange=()=>{{value.to_unit=to.value;pollingRunRows(container,count,runs)}};if(value.from_unit)used.add(value.from_unit);if(value.to_unit)used.add(value.to_unit);row.append(label,from,arrow,to);nodes.push(row);
    ...
    function pollingBindPersonSearch(input,results,excludedRecordIds=()=>[]) {{
      const matching=(people=litePollingPeople)=>{{const query=String(input.value||'').replace(/\\s+/g,'').toLowerCase(),building=pollingCurrentBuildingLabel(),currentShift=pollingCurrentShift(),excluded=new Set(excludedRecordIds().filter(Boolean).map(String));return [...people].filter(person=>!excluded.has(String(person.record_id||''))&&(!query||[person.name,person.employee_no,person.position,person.building,person.shift].join('').replace(/\\s+/g,'').toLowerCase().includes(query))).sort((a,b)=>(String(a.building||'').includes(building)?0:1)-(String(b.building||'').includes(building)?0:1)||(String(b.shift||'').includes(currentShift)?1:0)-(String(a.shift||'').includes(currentShift)?1:0)||String(a.employee_no||'').localeCompare(String(b.employee_no||''),'zh-CN')||String(a.name||'').localeCompare(String(b.name||''),'zh-CN')).slice(0,30)}};
      const render=(people=litePollingPeople)=>{{const matches=matching(people);results.hidden=false;if(!matches.length){{const empty=document.createElement('div');empty.className='empty compact';empty.textContent='没有匹配人员';results.replaceChildren(empty);return}}results.replaceChildren(...matches.map(person=>{{const button=document.createElement('button'),strong=document.createElement('strong'),small=document.createElement('small');button.type='button';button.className='polling-person-result';strong.textContent=person.name||'未命名人员';small.textContent=[person.employee_no,person.position,person.building,person.shift].filter(Boolean).join(' · ');button.append(strong,small);button.onclick=()=>{{pollingChoosePerson(input,person.record_id);results.hidden=true}};return button}}))}};
      input.onfocus=()=>render();input.oninput=()=>{{input.dataset.recordId='';render()}};input.onkeydown=event=>{{if(event.key==='Escape')results.hidden=true}};input.onblur=()=>window.setTimeout(()=>results.hidden=true,160);return {{render}};
    }}
    function renderPollingSelectionEditor() {{
      ...
      const work=pollingSopWorkType(),maintenance=work!=='polling',hasCoolingStep=work==='adjust'&&pollingSopIsAdjustCooling(sop),compatible=!pollingSopHasUnsupportedDevicePlaceholders(sop),syncBlocked=Boolean(sop.cloud_sync_required&&sop.cloud_sync_status!=='synced');const availability=document.createElement('div');availability.className='job-status';availability.textContent=syncBlocked?(sop.cloud_sync_error||'此 SOP 尚未同步到多维，请先点击下方“同步多维”。'):!compatible?'该 SOP 含当前工单不支持的设备指向占位符，不可用于维保或设备调整工单。':sop.ready?(hasCoolingStep?'SOP 已就绪，请选择制冷单元、当前模式、切换后模式和人员。':maintenance?'SOP 已就绪，请选择人员。':'SOP 已就绪，请配置设备指向和人员。'):(maintenance?'该 SOP 缺少步骤或附件，补充完整前不能确认。':'该 SOP 缺少步骤或附件，可先配置设备指向，但补充完整前不能确认。');
      if(!sop.ready||!compatible||(syncBlocked&&sop.cloud_sync_status==='conflict')){{const manage=document.createElement('button');manage.type='button';manage.className='btn ghost';manage.textContent=compatible&&sop.ready?'去修改 SOP':compatible?'去补充 SOP':'去修改 SOP';manage.onclick=()=>{{litePollingSopMode='manage';litePollingPendingSopFiles=[];litePollingEditingSop=structuredClone(sop);document.getElementById('lite-polling-sop-title').textContent='SOP步骤填写';document.getElementById('lite-polling-sop-new').hidden=false;renderPollingSopList();renderPollingSopEditor()}};availability.append(document.createTextNode(' '),manage)}}
      const preview=document.createElement('div'),previewRows=[];preview.className='polling-sop-steps';for(const step of sop.steps||[]){{const row=document.createElement('div');row.className='polling-step-edit polling-step-preview';previewRows.push([row,step]);preview.append(row)}}
      ...
      if(hasCoolingStep)syncCoolingSelection();apply.disabled=syncBlocked||!sop.ready||!compatible||(hasCoolingStep&&(!coolingUnit.value||!fromMode.value||!toMode.value||fromMode.value===toMode.value));apply.title=syncBlocked?'请先同步多维并处理冲突':!compatible?'该 SOP 含当前工单不支持的设备指向':!sop.ready?'请先补充 SOP 步骤和附件':hasCoolingStep&&apply.disabled?'请完成制冷单元模式切换配置':'确认当前选择';editor.append(availability,coolingFields,preview,countLabel,directionTitle,runs,peopleGrid,apply);
    }}
    ...
    function appendImagePreview(chip, item, alt) {{
      ...
      preview.type = 'button';
      preview.className = 'site-photo-preview';
      preview.title = `点击放大：${{alt}}`;
      ...
      preview.onclick = () => openChangeScreenshotPreview(previewUrl, alt);
    ...
    function persistAliImageOnRows(targetRecordId, items, freshForToday) {{
      ...
      document.querySelectorAll('.ongoing-row,.notice-row').forEach(row => {{
    ...
    function mergeNoticeImageItems(existing, local) {{
      const merged = [], tokenIndexes = new Map(), localIndexes = new Map(), hashIndexes = new Map();
      for (const item of [...(existing||[]),...(local||[])]) {{
        ...
        let index = token && tokenIndexes.has(token) ? tokenIndexes.get(token) : (localId && localIndexes.has(localId) ? localIndexes.get(localId) : (hash && hashIndexes.has(hash) ? hashIndexes.get(hash) : -1));
        ...
        if (hash) hashIndexes.set(hash,index);
      }}
      return merged;
    }}
    ...
    function updateSitePhotoUi(form) {{
      ...
      if (status) {{
        status.textContent = existing > 0 && !liteExistingSitePhotos.length
          ? `已有 ${{existing}} 张现场照片，本机无图片副本。`
          : total > 0
    ...
    async function uploadSitePhotoFile(file, form, kind = 'site', options = {{}}) {{
      if (!file || !form) return null;
      const imageLabel = kind === 'ali' ? '阿里确认截图' : '现场照片';
      if (!String(file.type || '').startsWith('image/')) throw new Error(`只能上传图片作为${{imageLabel}}。`);
      if (file.size > 8 * 1024 * 1024) throw new Error(`${{imageLabel}}不能超过 8MB。`);
      const identity = String(options.identity || localNoticeImageIdentity(form));
      const scope = String(options.scope || previewValue(form,'scope') || getCurrentScope());
      const targetRecordId = String(options.targetRecordId || '');
      const url = '/api/notice-attachments?file_name=' + encodeURIComponent(file.name || 'site_photo.png')
        ...
        + (targetRecordId ? '&target_record_id=' + encodeURIComponent(targetRecordId) : '');
      ...
      if (!response.ok || data.ok === false) throw new Error(data.error || data.message || `${{imageLabel}}上传失败`);
      const item = data.data || data;
      if (!item.upload_id) throw new Error(`${{imageLabel}}上传返回缺少 upload_id`);
      return {{
        ...item,
    ...
    function updateAliConfirmationUi(form) {{
      ...
      if (status) status.textContent = liteAliConfirmationImage?.remote_uploaded
        ...
        : (confirmed
          ? 'H楼已确认'
          : (existing
            ? (existingImages.length ? '截图已上传，等待H楼确认' : '截图已写入目标表，本机无图片副本，可重新上传')
            : '尚未上传'));
    ...
    async function uploadAliConfirmationNow(button) {{
      ...
      try {{
        ...
        const resultImages = (Array.isArray(result.local_screenshot_items)
          ? result.local_screenshot_items
          : []).filter(entry => entry?.target_written && String(entry?.preview_url || '').startsWith('/api/notice-images/'));
        const screenshotItems = resultImages.length
          ? resultImages
          : mergeNoticeImageItems(
    ...
    async function handleSitePhotoPaste(event) {{
      ...
      const confirmationModal = changeConfirmationModal();
      if (confirmationModal && !confirmationModal.hidden) {{
        const files = clipboardImageFiles(event);
        if (!files.length) {{
          setLiteStatus('剪贴板里没有可用图片');
          return;
        }}
        const focusedRecordId = String(
          target?.closest('[data-change-confirmation-upload-zone]')?.getAttribute('data-change-confirmation-upload-zone')
          || liteActiveChangeConfirmationRecordId
          || ''
        ).trim();
        event.preventDefault();
        if (!focusedRecordId) {{
          setLiteStatus('请先点击需要上传截图的变更通告');
          return;
        }}
        await stageChangeConfirmationFile(focusedRecordId, files[0]);
        return;
      }}
    ...
    const litePollingDelayCache = new Map();
    function refreshPollingDelayStatus(groupId, form) {{
      const cached=litePollingDelayCache.get(groupId),now=Date.now();
      if(cached?.loading||now-(cached?.loadedAt||0)<5000)return;
      litePollingDelayCache.set(groupId,{{...cached,loading:true}});
      pollingApi(`/api/workbench/polling-delay-status?group_id=${{encodeURIComponent(groupId)}}`).then(data=>{{litePollingDelayCache.set(groupId,{{data,loadedAt:Date.now(),loading:false}});if(form.isConnected)updateActionAvailability(form)}}).catch(error=>{{litePollingDelayCache.set(groupId,{{error:error.message||'状态读取失败',loadedAt:Date.now(),loading:false}});if(form.isConnected)updateActionAvailability(form)}});
    }}
    function updateActionAvailability(form) {{
      ...
      const groupId=targetForm.querySelector('[data-polling-group-id]')?.getAttribute('data-polling-group-id')||'';
      if(action==='end'&&groupId&&['maintenance','polling','adjust'].includes(workType)){{
        refreshPollingDelayStatus(groupId,targetForm);
        const status=litePollingDelayCache.get(groupId),delay=status?.data;
        if(!delay){{buttons.forEach(button=>button.disabled=true);setActionReason(status?.error?`工单状态读取失败：${{status.error}}`:'正在核验工单与延时提醒','blocked');return}}
        if(delay.state!=='completed'){{buttons.forEach(button=>button.disabled=true);setActionReason('工单尚未全部完成或附件尚未上传','blocked');return}}
        const remaining=Math.max(0,Math.ceil(Number(delay.due_at_ts||0)-Date.now()/1000));
        if(delay.missing_completion||remaining>0){{buttons.forEach(button=>button.disabled=true);const hours=Math.floor(remaining/3600),minutes=String(Math.floor(remaining%3600/60)).padStart(2,'0'),seconds=String(remaining%60).padStart(2,'0');setActionReason(delay.missing_completion?'延时提醒步骤尚未完成':`延时提醒剩余 ${{hours}}:${{minutes}}:${{seconds}}`,'blocked');return}}
      }}
    ...
    window.setInterval(()=>{{const form=document.getElementById('lite-notice-form');if(form?.dataset.action==='end'&&form.querySelector('[data-polling-group-id]'))updateActionAvailability(form)}},1000);
    ...
    function manualImageIdentityKey(form) {{
      if (!form || previewValue(form, 'manual') !== '1') return '';
      return `clipflow:manual-image:${{previewValue(form, 'scope') || getCurrentScope()}}:${{previewValue(form, 'work_type') || form.dataset.workType || ''}}`;
    }}
    function restoreManualImageIdentity(form) {{
      const key = manualImageIdentityKey(form);
      if (!key || previewValue(form, 'source_record_id') || previewValue(form, 'target_record_id')) return;
      try {{
        const stored = String(sessionStorage.getItem(key) || '').trim();
        const current = previewValue(form, 'manual_id');
        if (stored) setFormValue(form, 'manual_id', stored);
        else if (current) sessionStorage.setItem(key, current);
      }} catch {{}}
    }}
    function clearManualImageIdentityForLink(link) {{
      try {{
        const url = new URL(link.href, location.href);
        const scope = url.searchParams.get('scope') || getCurrentScope();
        const workType = url.searchParams.get('work_type') || '';
        sessionStorage.removeItem(`clipflow:manual-image:${{scope}}:${{workType}}`);
      }} catch {{}}
    }}
    function hydrateLitePreview() {{
      const form = document.getElementById('lite-notice-form');
      restoreManualImageIdentity(form);
    ...
    const liteChangeConfirmationDrafts = new Map();
    let liteActiveChangeConfirmationRecordId = '';
    ...
    function renderChangeConfirmations() {{
      ...
      const rows = visible.map(item => {{
        ...
        const localScreenshots = Array.isArray(item.local_screenshot_items)
          ? item.local_screenshot_items.filter(entry => entry?.target_written && String(entry?.preview_url || '').startsWith('/api/notice-images/'))
          : [];
        if (localScreenshots.length) {{
          const gallery = document.createElement('div');
          gallery.className = 'change-confirmation-gallery';
          localScreenshots.forEach((attachment, index) => {{
            const label = String(attachment.file_name || `阿里确认截图${{index + 1}}`);
            const url = String(attachment.preview_url || '');
            const button = document.createElement('button');
            const image = document.createElement('img');
            button.type = 'button';
            button.className = 'change-confirmation-thumb';
            button.title = `点击放大：${{label}}`;
            button.setAttribute('aria-label', button.title);
            image.src = url;
            image.alt = label;
            image.loading = 'lazy';
            button.append(image);
            button.onclick = () => openChangeScreenshotPreview(url, label);
            gallery.append(button);
          }});
          body.append(gallery);
        }} else if (Number(item.screenshot_count || 0) > 0) {{
          const missingLocal = document.createElement('div');
          missingLocal.className = 'change-confirmation-local-missing';
          missingLocal.textContent = '本机无图片副本，可重新上传。';
          body.append(missingLocal);
        }}
        if (item.last_error) {{
        ...
        const recordId = String(item.target_record_id || '');
        const pendingLocal = (Array.isArray(item.local_screenshot_items) ? item.local_screenshot_items : [])
          .filter(entry => !entry?.target_written)
          .slice(-1)[0] || null;
        if (!liteChangeConfirmationDrafts.has(recordId) && pendingLocal) {{
          liteChangeConfirmationDrafts.set(recordId, pendingLocal);
        }}
        const staged = liteChangeConfirmationDrafts.get(recordId) || null;
        const uploadZone = document.createElement('label');
        uploadZone.className = 'change-confirmation-upload-zone';
        uploadZone.tabIndex = 0;
        uploadZone.setAttribute('data-change-confirmation-upload-zone', recordId);
        uploadZone.classList.toggle('active', liteActiveChangeConfirmationRecordId === recordId);
        const input = document.createElement('input');
        input.type = 'file';
        input.accept = 'image/*';
        input.setAttribute('data-change-confirmation-file', recordId);
        const lead = document.createElement('span');
        lead.className = 'site-photo-add-icon';
        lead.textContent = '＋';
        const copy = document.createElement('span');
        copy.className = 'change-confirmation-upload-copy';
        const copyTitle = document.createElement('b');
        copyTitle.textContent = staged ? '已暂存' : '添加截图';
        const copyText = document.createElement('span');
        copyText.textContent = staged?.file_name || '点击或聚焦后 Ctrl+V';
        copy.append(copyTitle, copyText);
        uploadZone.append(input, lead, copy);
        uploadZone.onfocus = () => {{
          liteActiveChangeConfirmationRecordId = recordId;
          document.querySelectorAll('[data-change-confirmation-upload-zone]').forEach(zone =>
            zone.classList.toggle('active', zone === uploadZone)
          );
        }};
        actions.append(uploadZone);
        if (staged) {{
          const stagedChip = document.createElement('span');
          stagedChip.className = 'site-photo-item';
          appendImagePreview(stagedChip, staged, staged.file_name || '待上传阿里确认截图');
          const stagedLabel = document.createElement('span');
          stagedLabel.textContent = staged.file_name || '待上传阿里确认截图';
          stagedChip.append(stagedLabel);
          actions.append(stagedChip);
          const upload = document.createElement('button');
          upload.type = 'button';
          upload.className = 'btn primary compact-upload';
          upload.textContent = '上传';
          upload.setAttribute('data-change-confirmation-upload', recordId);
          const remove = document.createElement('button');
          remove.type = 'button';
          remove.className = 'btn ghost compact-upload';
          remove.textContent = '移除';
          remove.setAttribute('data-change-confirmation-draft-remove', recordId);
    ...
    async function loadChangeConfirmations() {{
      ...
      try {{
        ...
        for (const item of liteChangeConfirmationItems) {{
          const recordId = String(item?.target_record_id || '');
          const staged = liteChangeConfirmationDrafts.get(recordId);
          if (
            staged?.local_image_id
            && (item.local_screenshot_items || []).some(entry =>
              entry?.target_written
              && String(entry.local_image_id || '') === String(staged.local_image_id)
            )
    ...
    async function stageChangeConfirmationFile(recordId, file) {{
      recordId = String(recordId || '').trim();
      if (!recordId || !file) return;
      const previous = liteChangeConfirmationDrafts.get(recordId);
      const uploaded = await uploadSitePhotoFile(file, document.body, 'ali', {{
        identity: `change:${{recordId}}`,
        targetRecordId: recordId,
        scope: getCurrentScope(),
      }});
      if (!uploaded?.local_image_id) throw new Error('本地截图保存失败');
      if (uploaded.target_written) {{
        liteChangeConfirmationDrafts.delete(recordId);
        await loadChangeConfirmations();
        setLiteStatus('该截图已经上传，无需重复提交');
        return;
      }}
      liteChangeConfirmationDrafts.set(recordId, uploaded);
      liteActiveChangeConfirmationRecordId = recordId;
      if (
        previous?.local_image_id
        && previous.local_image_id !== uploaded.local_image_id
        && !previous.target_written
      ) {{
        fetch(`/api/notice-images/${{encodeURIComponent(previous.local_image_id)}}`, {{
          method: 'DELETE', credentials: 'same-origin'
        }}).catch(() => null);
      }}
      renderChangeConfirmations();
      setLiteStatus('截图已在本机暂存，请确认后点击上传');
    }}
    async function uploadChangeConfirmationFromDashboard(input) {{
      ...
      input.disabled = true;
      try {{
        await stageChangeConfirmationFile(recordId, file);
      }} catch (error) {{
        showLiteError(error && error.message ? error.message : '阿里确认截图暂存失败');
      }} finally {{
        input.disabled = false;
        input.value = '';
      }}
    }}
    function applyChangeConfirmationLocalImages(item) {{
      const recordId = String(item?.target_record_id || '').trim();
      if (!recordId) return;
      const images = (Array.isArray(item?.local_screenshot_items) ? item.local_screenshot_items : [])
        .filter(entry => entry?.target_written && String(entry?.preview_url || '').startsWith('/api/notice-images/'));
      persistAliImageOnRows(recordId, images, item?.screenshot_fresh_for_today === true);
      const form = document.getElementById('lite-notice-form');
      const panel = form?.querySelector('[data-ali-confirmation-panel]');
      if (!panel || String(panel.dataset.targetRecordId || '') !== recordId) return;
      panel.dataset.existingImages = JSON.stringify(images);
      panel.dataset.existingCount = String(Math.max(Number(item?.screenshot_count || 0), images.length));
      panel.dataset.confirmed = item?.h_confirmed ? '1' : '0';
      panel.dataset.freshForToday = item?.screenshot_fresh_for_today ? '1' : '0';
      updateAliConfirmationUi(form);
    }}
    async function uploadChangeConfirmationStaged(button) {{
      const recordId = String(button?.getAttribute('data-change-confirmation-upload') || '').trim();
      const staged = liteChangeConfirmationDrafts.get(recordId);
      if (!recordId || !staged?.local_image_id) return;
      setButtonBusy(button, true);
      try {{
        const response = await fetch(`/api/change-confirmations/${{encodeURIComponent(recordId)}}/screenshot`, {{
          method: 'POST', credentials: 'same-origin', headers: {{ 'Content-Type': 'application/json' }},
          body: JSON.stringify({{
            upload_id: String(staged.upload_id || ''),
            local_image_id: String(staged.local_image_id || ''),
          }}),
        }});
        const data = await response.json().catch(() => ({{}}));
        if (handleLiteAuthRequired(response, data)) return;
    ...
    async function removeChangeConfirmationDraft(button) {{
      const recordId = String(button?.getAttribute('data-change-confirmation-draft-remove') || '').trim();
      const staged = liteChangeConfirmationDrafts.get(recordId);
      if (!recordId || !staged) return;
      setButtonBusy(button, true);
      try {{
        if (staged.local_image_id) {{
          const response = await fetch(`/api/notice-images/${{encodeURIComponent(staged.local_image_id)}}`, {{
            method: 'DELETE', credentials: 'same-origin'
          }});
          const data = await response.json().catch(() => ({{}}));
          if (!response.ok || data.ok === false) throw new Error(data.error || '本地截图删除失败');
        }}
        liteChangeConfirmationDrafts.delete(recordId);
        renderChangeConfirmations();
        setLiteStatus('已移除待上传截图');
      }} catch (error) {{
        showLiteError(error && error.message ? error.message : '本地截图删除失败');
      }} finally {{
        setButtonBusy(button, false);
      }}
    }}
    ...
    document.addEventListener('click', async (event) => {{
      ...
      if (pollingSopDeleteApply) {{ event.preventDefault(); await confirmPollingSopDelete(pollingSopDeleteApply); return; }}
      const pollingSopDeleteBackdrop = target.closest('#lite-polling-sop-delete-confirm');
      ...
      const changeConfirmationUploadZone = target.closest('[data-change-confirmation-upload-zone]');
      if (changeConfirmationUploadZone) {{
        liteActiveChangeConfirmationRecordId = String(changeConfirmationUploadZone.getAttribute('data-change-confirmation-upload-zone') || '');
        document.querySelectorAll('[data-change-confirmation-upload-zone]').forEach(zone =>
          zone.classList.toggle('active', zone === changeConfirmationUploadZone)
        );
      }}
      const changeConfirmationUpload = target.closest('[data-change-confirmation-upload]');
      if (changeConfirmationUpload) {{ event.preventDefault(); await uploadChangeConfirmationStaged(changeConfirmationUpload); return; }}
      const changeConfirmationDraftRemove = target.closest('[data-change-confirmation-draft-remove]');
      if (changeConfirmationDraftRemove) {{ event.preventDefault(); await removeChangeConfirmationDraft(changeConfirmationDraftRemove); return; }}
    ...
    document.addEventListener('change', async (event) => {{
      ...
      if (event.target && event.target.id === 'lite-polling-work-order-exempt') {{
        ...
        if (event.target.checked) litePollingSelection = null;
        updatePollingSelectionSummary(form);
        setLiteFormDirty(true);
        updateNoticePreview(form);
        updateActionAvailability(form);
        return;
    ...
    function promoteSubmittedStartToOngoing(draft) {{
      ...
      const aliPanel = form.querySelector('[data-ali-confirmation-panel]');
      if (aliPanel) aliPanel.dataset.targetRecordId = targetRecordId;
      if (liteAliConfirmationImage?.staged) liteAliConfirmationImage = null;
      if (draftSourceId) {{
...
q('retry').onclick=retryUpload;q('cancel-selection').onclick=cancelSelection;if(!token)status('工单链接缺少令牌。','error');else{load();timer=setInterval(load,3000)}
</script></body></html>"""
...
async function uploadStepPhoto(step,file){if(!current||busy||!file)return;busy=true;let message='',tone='';render(current);try{const url=`/api/polling-work-orders/photo?token=${encodeURIComponent(token)}&step_key=${encodeURIComponent(step.step_key)}&expected_version=${current.version}&file_name=${encodeURIComponent(file.name||'step_photo.png')}`,r=await fetch(url,{method:'POST',headers:{'Content-Type':file.type||'image/png'},body:file}),body=await r.json();if(!r.ok||body.ok===false)throw new Error(body.error||'操作照片上传失败');current=body.data;receivedAt=Date.now();message=`操作照片已保存，共 ${(current.steps||[]).find(item=>item.step_key===step.step_key)?.photos?.length||0} 张`;tone='success'}catch(error){message=error.message||'操作照片上传失败';tone='error'}finally{busy=false;if(current&&!navigating)render(current);if(!navigating)status(message,tone)}}
async function confirmStep(stepKey){if(!current||busy)return;busy=true;let message='';render(current);try{const r=await fetch('/api/polling-work-orders/confirm',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,step_key:stepKey,expected_version:current.version})}),body=await r.json();if(!r.ok||body.ok===false)throw new Error(body.error||'确认失败');receivedAt=Date.now();const session=body.data.session;if(Number(session?.current_run_index||0)!==runIndex){goOverview();return}current=session;render(current)}catch(error){message=error.message||'确认失败'}finally{busy=false;if(message&&!navigating){if(current)render(current);status(message,'error')}}}
async function rollbackPrevious(stepKey){if(!current||busy||current.role!=='reviewer'||!current.can_rollback_previous)return;if(!window.confirm('确定回退至上一步吗？\n\n回退后，上一步和当前步骤的确认记录、操作照片将被清空；请按步骤要求重新执行并确认。'))return;busy=true;let message='',tone='';render(current);try{const r=await fetch('/api/polling-work-orders/rollback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,step_key:stepKey,expected_version:current.version})}),body=await r.json();if(!r.ok||body.ok===false)throw new Error(body.error||'回退失败');current=body.data;receivedAt=Date.now();message='已回退至上一步，请按步骤要求重新执行并确认。';tone='success'}catch(error){message=error.message||'回退失败';tone='error'}finally{busy=false;if(current&&!navigating)render(current);if(message&&!navigating)status(message,tone)}}
q('back').onclick=leaveWorkOrder;if(!token)status('工单链接缺少令牌。','error');else if(!Number.isInteger(runIndex)||runIndex<1||runIndex>6)goOverview();else{load();timer=setInterval(load,3000);clock=setInterval(refreshCountdown,1000)}
</script></body></html>"""
