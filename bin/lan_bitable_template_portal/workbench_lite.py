...
<body>
  ...
  <script>
    ...
    function pollingNewDraft() {{ return {{sop_id:'',work_type:pollingSopWorkType(),scope:pollingSopScope(),name:'',version:0,steps:[{{step_id:'',content:'',operator_required:true,reviewer_required:true,time_limit_seconds:0}}],attachments:[]}}; }}
    ...
    function renderPollingSelectionEditor() {{
      ...
      const maintenance=pollingSopWorkType()==='maintenance';const availability=document.createElement('div');availability.className='job-status';availability.textContent=sop.ready?(maintenance?'SOP 已就绪，请选择人员。':'SOP 已就绪，请配置设备指向和人员。'):(maintenance?'该 SOP 缺少步骤或附件，补充完整前不能确认。':'该 SOP 缺少步骤或附件，可先配置设备指向，但补充完整前不能确认。');
    ...
    async function openPollingSopModal(mode='manage') {{
      ...
      try{{
        await loadPollingSops();litePollingEditingSop=litePollingEditingSop&&litePollingEditingSop.scope===pollingSopScope()&&String(litePollingEditingSop.work_type||'polling')===pollingSopWorkType()?litePollingEditingSop:pollingNewDraft();litePollingSelectedSop=mode==='select'?(litePollingSops.find(item=>item.sop_id===litePollingSelection?.sop_id&&String(item.work_type||'polling')===pollingSopWorkType())||null):litePollingSelectedSop;renderPollingSopList();
