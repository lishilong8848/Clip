    ...
    return """
      <section class="form-section polling-work-order-select" data-polling-work-order-select>
        ...
        <div class="polling-work-order-actions">
          ...
          <label class="polling-work-order-exempt"><input id="lite-polling-work-order-exempt" name="polling_work_order_exempt" type="checkbox" value="1"><span>非制冷单元/二次泵轮巡</span></label>
        </div>
      </section>
    """
...
<body>
  ...
  <script>
    ...
    function updatePollingSelectionSummary(form) {{const section=form?.querySelector('[data-polling-work-order-select]');if(!section)return;const start=String(form.dataset.action||'start')==='start',exempt=pollingWorkOrderExempt(form),valid=litePollingSelection&&litePollingSelection.identity===sitePhotoSignature(form),select=document.getElementById('lite-polling-select-open'),summary=document.getElementById('lite-polling-selection-summary'),toggle=form?.querySelector('.polling-work-order-exempt');section.hidden=!start;if(toggle)toggle.classList.toggle('active',exempt);if(select){{select.disabled=exempt;select.title=exempt?'当前轮巡不启用工单流程':''}}if(summary)summary.textContent=exempt?'已忽略工单流程':valid?`${{litePollingSelection.sop_name}} · ${{litePollingSelection.runs.map(run=>`${{run.from_unit}}→${{run.to_unit}}`).join('、')}} · ${{litePollingSelection.operator_name}}/${{litePollingSelection.reviewer_name}}`:'未选择 SOP';}}
    ...
    function formPayload(form, submitter, actionOverride) {{
      ...
      if (patch.work_type === 'polling' && action === 'start') {{
        patch.polling_work_order_exempt = pollingWorkOrderExempt(form);
        if (!patch.polling_work_order_exempt && litePollingSelection && litePollingSelection.identity === sitePhotoSignature(form)) {{
...
async function rollbackPrevious(stepKey){if(!current||busy||current.role!=='reviewer'||!current.can_rollback_previous)return;if(!window.confirm('回退后，上一步和当前步骤的确认及照片会清空，需要重新拍照执行。确定回退吗？'))return;busy=true;let message='';render(current);try{const r=await fetch('/api/polling-work-orders/rollback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,step_key:stepKey,expected_version:current.version})}),body=await r.json();if(!r.ok||body.ok===false)throw new Error(body.error||'回退失败');current=body.data;receivedAt=Date.now();render(current);status('已回退至上一步，请重新拍照并确认。','success')}catch(error){message=error.message||'回退失败'}finally{busy=false;if(message&&!navigating){if(current)render(current);status(message,'error')}}}
