...
async function rollbackPrevious(stepKey){if(!current||busy||current.role!=='reviewer'||!current.can_rollback_previous)return;if(!window.confirm('确定回退至上一步吗？\n\n回退后，上一步和当前步骤的确认记录、操作照片将被清空；上一步需重新倒计时、拍照并确认。'))return;busy=true;let message='',tone='';render(current);try{const r=await fetch('/api/polling-work-orders/rollback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,step_key:stepKey,expected_version:current.version})}),body=await r.json();if(!r.ok||body.ok===false)throw new Error(body.error||'回退失败');current=body.data;receivedAt=Date.now();message='已回退至上一步，请重新拍照并确认。';tone='success'}catch(error){message=error.message||'回退失败';tone='error'}finally{busy=false;if(current&&!navigating)render(current);if(message&&!navigating)status(message,tone)}}
