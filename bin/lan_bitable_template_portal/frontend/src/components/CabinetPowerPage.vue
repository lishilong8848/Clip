<template>
  <main class="cabinet-page">
    <header class="heading">
      ...
      <div class="actions">
        <button :disabled="loading || busy" @click="refresh"><RefreshCw :size="16" :class="{ spin: loading || busy }" />刷新</button>
        <template v-if="scope">
    ...
    <div v-if="loading" class="notice" role="status"><Loader2 class="spin" :size="18" />正在读取台账…</div>
...
const overview = ref<Dict>({}), buildings = ref<Dict[]>([]), loading = ref(false), error = ref(''), message = ref('');
...
let disposed = false, pollTimer: number | undefined, recordSequence = 0, mapSequence = 0, historySequence = 0;
...
async function refresh(): Promise<void> { if (props.scope) await startJob('refresh'); else await load(); }
...
onBeforeUnmount(() => { persistDraft(); disposed = true; removeNavigationGuard?.(); window.removeEventListener('pagehide', persistDraft); window.removeEventListener('keydown', keyboard); window.clearTimeout(pollTimer); window.clearTimeout(savePollTimer); });
</script>
