<template>
  <main class="cabinet-page">
    ...
    <div v-if="bootstrapVisible" class="notice bootstrap-notice" :class="{ danger: bootstrapFailed }" role="status" aria-live="polite">
      ...
      <button v-if="bootstrapHasFailures" :disabled="bootstrapActive" @click="startBootstrap">重试初始化</button>
    </div>
    ...
    <section v-if="!scope" class="buildings">
      <button v-for="building in buildings" :key="building.scope" class="building" @click="navigate('/cabinet-power?scope=' + building.scope)">
        <div class="building-title"><Building2 :size="26" /><h2>{{ building.scope }}楼</h2><ArrowUpRight :size="18" /></div>
        <strong>{{ building.counts.total }}<small>机柜</small></strong>
        <div class="building-stats"><span>正式电<b>{{ building.counts.formal }}</b></span><span>测试电<b>{{ building.counts.test }}</b></span><span>未上电/已下电<b>{{ building.counts.off }}</b></span></div>
        <p>{{ building.record_count }} 条台账 · {{ building.inventory_only }} 柜无操作 · {{ building.counts.unknown }} 柜待核实</p>
      </button>
    </section>
...
let disposed = false, pollTimer: number | undefined, bootstrapTimer: number | undefined, recordSequence = 0, mapSequence = 0, historySequence = 0, initialDataLoaded = false;
function fail(exc: any): void { error.value = exc?.message || '读取失败，请重试'; }
async function applyBootstrap(data: Dict): Promise<void> {
  bootstrap.value = data;
  if (bootstrapTargetReady.value && !initialDataLoaded) { await load(); initialDataLoaded = props.scope ? Boolean(overview.value.rooms) : Boolean(buildings.value.length); }
...
async function pollBootstrap(): Promise<void> {
  ...
  catch (exc) { fail(exc); if (!disposed) bootstrapTimer = window.setTimeout(pollBootstrap,4000); }
}
async function startBootstrap(): Promise<void> {
  ...
  try { await applyBootstrap(await write('bootstrap',{})); }
  catch (exc) { bootstrap.value = {}; fail(exc); }
}
...
async function refresh(): Promise<void> { if (!initialDataLoaded || bootstrapHasFailures.value) await startBootstrap(); else if (props.scope) await startJob('refresh'); else await load(); }
...
.metric-link{border:0;background:transparent;border-radius:0;display:flex;flex-direction:column;align-items:flex-start;width:100%;padding:0}.carrier-summary>div{display:flex;align-items:center;gap:20px;padding:16px 0;border-bottom:1px solid #dce6f1}.rack-map-link{margin-left:20px;color:#60768c}
