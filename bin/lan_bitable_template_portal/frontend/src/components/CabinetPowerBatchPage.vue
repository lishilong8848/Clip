<template>
  <main class="batch-page" :class="{ 'has-detail': batchId, 'is-text-entry': mode==='new' && !batchId && createMode==='text' }">
    <header class="page-heading">
      ...
      <div class="page-title"><h1>{{ batchId ? '上下电待办详情' : mode === 'new' ? '机柜批量登记' : '上下电待办' }}</h1></div>
      <div class="actions">
        <button v-if="mode !== 'new' && !batchId" class="primary" @click="openNew"><Files :size="16" />批量登记</button>
        <button v-if="batchId && params.get('origin') === 'cabinet'" @click="openList"><ClipboardList :size="16" />待办列表</button>
