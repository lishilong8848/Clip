<template>
  <main class="batch-page">
    ...
    <template v-else-if="batchId && batch.batch_id">
      ...
      <section class="evidence-section"><div class="section-heading"><h3>确认截图</h3><span>{{ activeImages.length }} 张</span></div><div class="evidence-upload" :class="{ dragging: evidenceDragging }" @dragover.prevent="evidenceDragging = true" @dragleave.prevent="evidenceDragging = false" @drop.prevent="dropEvidence"><Upload :size="20" /><span>拖入截图或按 Ctrl+V 粘贴</span><label class="file-command">选择图片<input ref="imageInput" type="file" accept="image/jpeg,image/png,image/webp" multiple @change="selectEvidence" /></label><label class="file-command">选择文件夹<input type="file" accept="image/jpeg,image/png,image/webp" multiple webkitdirectory @change="selectEvidence" /></label><small v-if="uploadingEvidence">已上传 {{ imageUploadDone }}/{{ imageUploadTotal }} 张，正在识别…</small></div>
        <div v-if="activeImages.length" class="evidence-grid">
          <article v-for="image in activeImages" :key="image.image_id" class="evidence-item">
      ...
      <section v-if="batch.rows?.length" class="batch-actions">
        ...
        <div class="commit-actions"><span v-if="saving || dirtyCount" class="save-indicator">{{ saving ? '处理中…' : `待自动保存 ${dirtyCount} 条` }}</span><button v-if="saveFailed" @click="saveChanges">重试保存</button><label><span>按楼确认</span><select v-model="confirmScope" :disabled="batch.status === 'cancelled'"><option value="">选择楼栋</option><option v-for="item in batch.allowed_scopes || []" :key="item" :value="item">{{ item }}楼</option></select></label><button :disabled="saving || batch.status === 'cancelled' || !confirmScope || warningsPending" @click="confirmRows({ scope: confirmScope })">确认本楼</button><button :disabled="saving || batch.status === 'cancelled' || !selectedConfirmableCount || warningsPending" @click="confirmRows({ row_ids: selectedRows })">确认已选</button><button class="primary" :disabled="saving || batch.status === 'cancelled' || !batch.stats?.confirmable || !batch.can_confirm_all || warningsPending" :title="warningsPending ? '请先核对通告明细异常' : batch.can_confirm_all ? '' : '整批确认需要拥有全部楼栋权限'" @click="confirmDialog = 'all'">确认整批</button><button :disabled="saving || !rollbackableCount || !batch.can_confirm_all" @click="requestRollback('')">回退本批已写入记录</button><button :disabled="saving || batch.status === 'completed' || batch.status === 'cancelled' || !cancellableCount" @click="confirmDialog = 'cancel'">{{ batch.status === 'cancelled' ? '已作废' : '作废未提交行' }}</button></div>
      </section>
    ...
    <template v-else>
      <section class="filters"><select v-model="listScope"><option value="">全部楼栋</option><option v-for="item in buildingOptions" :key="item.value" :value="item.value">{{ item.label }}</option></select><select v-model="listStatus"><option value="todo">仅看待办</option><option value="">全部状态</option><option value="pending">待处理</option><option value="running">提交中</option><option value="partial">部分完成</option><option value="completed">已完成</option><option value="rolled_back">已回退</option><option value="cancelled">已作废</option><option value="failed">失败</option></select><input v-model="listFrom" type="date" aria-label="开始日期" /><input v-model="listTo" type="date" aria-label="结束日期" /><button @click="loadList(1)"><Search :size="16" />查询</button></section>
      <div class="table-wrap"><table class="batch-list-table"><thead><tr><th>批次</th><th>来源</th><th>楼栋</th><th>状态</th><th>总数</th><th>待处理</th><th>可确认</th><th>异常</th><th>创建时间</th><th>操作</th></tr></thead><tbody><tr v-for="item in list.items || []" :key="item.batch_id" :class="{ 'todo-batch': item.is_todo }"><td><strong>{{ item.batch_id.slice(0,8) }}</strong><span v-if="item.is_todo" class="todo-mark">待办</span></td><td>{{ sourceLabel(item.source) }}</td><td>{{ (item.scopes || []).map((value: string) => value + '楼').join('、') || '识别中' }}</td><td><span :class="'status ' + item.status">{{ statusLabel(item.status) }}</span></td><td>{{ item.stats?.total || 0 }}</td><td><strong v-if="item.pending_rows" class="todo-count">{{ item.pending_rows }}</strong><span v-else-if="item.is_todo">{{ item.pending_label }}</span><span v-else>—</span></td><td>{{ item.stats?.confirmable || 0 }}</td><td>{{ (item.stats?.duplicate || 0) + (item.stats?.conflict || 0) + (item.stats?.invalid || 0) + (item.stats?.failed || 0) }}</td><td>{{ item.created_at }}</td><td><button class="link" @click="openBatch(item.batch_id)">{{ item.is_todo ? '处理待办' : '查看历史' }}</button></td></tr></tbody></table><p v-if="!loading && !list.total" class="empty">{{ listStatus === 'todo' ? '当前没有待处理批次，可切换“全部状态”查看历史。' : '暂无上下电待办' }}</p></div>
...
const fieldLabels: Dict = {scope:'楼栋',room:'包间',rack:'机柜',supplier_rack:'供应商机柜号',rack_type:'机柜类型',type_detail:'类型明细',action:'操作类型',expected:'期望完成时间',actual:'实际完成时间',result:'结果',failure_reason:'失败原因',type_resolution:'类型处理',excluded:'排除状态'};
...
const selectedRows = ref<string[]>([]), editingRowId = ref(''), confirmScope = ref(props.scope || ''), rowPage = ref(1), rowScopeFilter = ref(props.scope || ''), rowStatusFilter = ref(''), listScope = ref(props.scope || ''), listStatus = ref(params.get('status')==='todo'?'todo':''), listFrom = ref(''), listTo = ref('');
...
const selectedConfirmableCount = computed(() => (batch.value.rows || []).filter((row:Dict)=>selectedRows.value.includes(row.row_id)&&row.confirmable).length);
...
async function loadBatch(): Promise<void> {
  ...
  try { batch.value=await read(`batches/${batchId}`); snapshotRows(); syncImageSelections(); error.value=''; if (['recognizing','running'].includes(batch.value.status) || batch.value.images?.some((item:Dict)=>!item.deleted_at&&item.status==='recognizing')) schedulePoll(); }
...
watch([rowScopeFilter,rowStatusFilter],()=>{rowPage.value=1;if(editingRowId.value)closeRowEditor();});
...
onMounted(async()=>{window.addEventListener('paste',pasteFiles);window.addEventListener('keydown',closeImageOnEscape);removeGuard=registerNavigationGuard((_target,proceed)=>{if(!dirtyCount.value&&!saving.value)return true;void saveChanges().then(ok=>{if(ok&&!dirtyCount.value)proceed();else{pendingNavigation=proceed;discardOpen.value=true;}});return false;});if(batchId)await loadBatch();else if(mode!=='new')await loadList();if(mode==='new'&&createMode.value==='manual')await loadDirectory();});
onBeforeUnmount(()=>{disposed=true;window.clearTimeout(pollTimer);window.clearTimeout(saveTimer);window.removeEventListener('paste',pasteFiles);window.removeEventListener('keydown',closeImageOnEscape);for(const url of imagePreviewUrls.values())URL.revokeObjectURL(url);imagePreviewUrls.clear();removeGuard?.();});
</script>
...
.evidence-match select{max-width:260px}
