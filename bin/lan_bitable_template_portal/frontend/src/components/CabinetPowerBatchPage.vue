  ...
  <main class="batch-page">
    ...
    <template v-if="mode === 'new' && !batchId">
      ...
      <section v-if="createMode === 'pdf'" class="entry-section">
        ...
        <label class="file-drop"><Upload :size="24" /><strong>选择PDF确认单</strong><input type="file" accept="application/pdf,.pdf" multiple @change="selectFiles" /></label>
        <ul v-if="pdfFiles.length" class="file-list"><li v-for="file in pdfFiles" :key="file.name + file.size"><FileText :size="16" /><span>{{ file.name }}</span><small>{{ formatBytes(file.size) }}</small></li></ul>
    ...
    <template v-else-if="batchId && batch.batch_id">
      ...
      <section v-if="batch.rows?.length" class="batch-actions">
        ...
        <div class="actions"><button :disabled="saving || !dirtyCount" @click="saveChanges"><Save :size="16" />保存更正<span v-if="dirtyCount">（{{ dirtyCount }}）</span></button><select v-model="confirmScope" aria-label="按楼确认"><option value="">选择楼栋</option><option v-for="item in batch.allowed_scopes || []" :key="item" :value="item">{{ item }}楼</option></select><button :disabled="saving || !confirmScope" @click="confirmRows({ scope: confirmScope })">确认本楼</button><button :disabled="saving || !selectedRows.length" @click="confirmRows({ row_ids: selectedRows })">确认已选</button><button class="primary" :disabled="saving || !batch.stats?.confirmable || !batch.can_confirm_all" :title="batch.can_confirm_all ? '' : '整批确认需要拥有全部楼栋权限'" @click="confirmDialog = 'all'">确认整批</button><button :disabled="saving || batch.status === 'completed'" @click="confirmDialog = 'cancel'">作废未提交行</button></div>
      </section>
      ...
      <div v-if="batch.rows?.length" class="table-wrap detail-table"><table><thead><tr><th><input type="checkbox" :checked="allVisibleSelected" aria-label="选择本页" @change="toggleVisible" /></th><th>状态</th><th>楼栋</th><th>包间</th><th>机柜</th><th>供应商机柜号</th><th>机柜类型</th><th>类型明细</th><th>操作类型</th><th>期望完成时间</th><th>实际完成时间</th><th>结果</th><th>下单时间</th><th>来源</th><th>操作</th></tr></thead><tbody>
        <tr v-for="row in pagedRows" :key="row.row_id" :class="row.status">
          ...
          <td><select v-model="row.action" :disabled="!row.editable" :class="{ corrected: corrected(row,'action') }"><option v-for="action in actions" :key="action">{{ action }}</option></select></td>
          ...
          <td><select v-model="row.result" :disabled="!row.editable" :class="{ corrected: corrected(row,'result') }"><option>成功</option><option>失败</option></select></td>
    ...
    <template v-else>
      ...
      <div class="table-wrap"><table><thead><tr><th>批次</th><th>来源</th><th>楼栋</th><th>状态</th><th>总数</th><th>可确认</th><th>异常</th><th>创建时间</th><th>操作</th></tr></thead><tbody><tr v-for="item in list.items || []" :key="item.batch_id"><td>{{ item.batch_id.slice(0,8) }}</td><td>{{ item.source === 'pdf' ? 'PDF识别' : '手工批量' }}</td><td>{{ (item.scopes || []).map((value: string) => value + '楼').join('、') }}</td><td><span :class="'status ' + item.status">{{ statusLabel(item.status) }}</span></td><td>{{ item.stats?.total || 0 }}</td><td>{{ item.stats?.confirmable || 0 }}</td><td>{{ (item.stats?.duplicate || 0) + (item.stats?.conflict || 0) + (item.stats?.invalid || 0) + (item.stats?.failed || 0) }}</td><td>{{ item.created_at }}</td><td><button class="link" @click="openBatch(item.batch_id)">查看处理</button></td></tr></tbody></table><p v-if="!loading && !list.total" class="empty">暂无上下电待办</p></div>
...
const createMode = ref('pdf'), pdfFiles = ref<File[]>([]), manualScope = ref(props.scope || buildingOptions.value[0]?.value || 'A');
...
const confirmTitle = computed(() => confirmDialog.value === 'overlap' ? '清空完全重叠的待办行？' : confirmDialog.value === 'all' ? '确认整批有效记录？' : confirmDialog.value === 'file' ? '清理原确认单？' : '作废尚未提交的记录？');
const confirmMessage = computed(() => confirmDialog.value === 'overlap' ? '只排除当前批次中的重复行，既有台账和多维表不会被修改；排除记录仍保留审计并可恢复。' : confirmDialog.value === 'all' ? `将并行确认各楼共 ${batch.value.stats?.confirmable || 0} 条有效记录，异常行会留在待办。` : confirmDialog.value === 'file' ? '清理后无法再次下载原PDF，已保存的识别值、更正审计和正式台账不受影响。' : '已完成记录保留，其他尚未提交的行将标记为作废。');
const confirmLabel = computed(() => confirmDialog.value === 'overlap' ? '清空重叠数据' : confirmDialog.value === 'all' ? '确认整批' : confirmDialog.value === 'file' ? '清理原确认单' : '作废未提交行');
...
function selectFiles(event: Event): void { pdfFiles.value = Array.from((event.target as HTMLInputElement).files || []).slice(0,10); if ((event.target as HTMLInputElement).files?.length && pdfFiles.value.length < (event.target as HTMLInputElement).files!.length) error.value='每批最多选择10份PDF。'; }
