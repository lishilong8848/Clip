...
NOTICE_TYPE_BIANGENG = "变更通告"  # 变更通告
...
def _extract_event_building_key(text: str, title: str) -> str:
    def _codes_from_text(value: str) -> list[str]:
        ...
        matches: list[str] = []
        for item in re.findall(r"([A-EH])\s*楼", normalized):
            code = item.strip().upper()
            if code and code not in matches:
                matches.append(code)
        if "110" in normalized and "110" not in matches:
            matches.insert(0, "110")
        return matches

    matches = _codes_from_text(title)
    if matches:
        return ",".join(matches)
    matches = _codes_from_text(text)
    return ",".join(matches)
...
def extract_notice_info(content):
    ...
    if notice_type in {NOTICE_TYPE_BIANGENG, NOTICE_TYPE_SHIJIAN}:
        ...
        if valid_level_match:
            level = valid_level_match.group(1).strip()

    # 维保可能同名称同时间但原因不同，原因必须参与身份匹配。
    if notice_type == NOTICE_TYPE_SHIJIAN:
        building_key = _extract_event_building_key(raw_content, title)
        unique_key = f"{title}|{time_str}|{building_key}|{source}|{level or ''}"
    elif notice_type == NOTICE_TYPE_WEIBAO and reason:
        unique_key = f"{title}|{time_str}|{reason}"
    else:
